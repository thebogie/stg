use arangors::{Database, client::ClientExt};
use log;
...
#[derive(Debug, Clone)]
struct GooglePlacesMigrationService {
    api_url: String,
    api_key: String,
...
pub async fn migrate_venues_place_id(
    ...
    // Find all venues with null or empty place_id
    let query = arangors::AqlQuery::builder()
        .query(r#"
            FOR venue IN venue
            FILTER venue.place_id == null || venue.place_id == "" || venue.place_id == "Unknown Place"
            RETURN venue
        "#)
        .build();
                    ...
                    Ok(Some(place_id)) => {
                        ...
                        let update_query = arangors::AqlQuery::builder()
                            .query(r#"
                                ...
                                RETURN NEW
                            "#)
                            .bind_var("venue_id", &venue.id)
                            .bind_var("place_id", &place_id)
                            .build();

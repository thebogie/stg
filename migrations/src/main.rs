...
impl Arango {
	async fn authenticate(endpoint: &str, db: &str, username: &str, password: &str) -> Result<Self> {
		let base = Url::parse(endpoint).context("Invalid endpoint URL")?;
		let http = reqwest::Client::builder().build()?;
		#[derive(Deserialize)]
		struct AuthResp { jwt: String }
		...
		Ok(Self { base, db: db.to_string(), jwt, http })
	}
	...
	async fn ensure_document_collection(&self, name: &str, dry: bool) -> Result<()> {
		let get = self.auth(self.http.get(self.db_url(&format!("/_api/collection/{name}"))?)).send().await?;
		...
		let create = self.auth(self.http.post(self.db_url("/_api/collection")?))
		...
		if !status.is_success() {
			...
			return Err(anyhow!("Create collection {} failed: {} - {}", name, status, txt));
		}
		Ok(())
	}

	async fn create_collection_with_options(&self, name: &str, kind: Option<String>, options: Option<serde_json::Value>, dry: bool) -> Result<()> {
		...
		if dry {
			println!("[dry-run] create collection {} (kind={:?}) opts={}", name, kind, options.as_ref().map(|v| v.to_string()).unwrap_or_else(|| "null".into()));
			return Ok(());
		}
		...
		println!("Collection creation body: {}", serde_json::to_string_pretty(&body)?);
	...
	async fn ensure_index(&self, collection: &str, index_body: serde_json::Value, dry: bool) -> Result<()> {
		if dry {
		...
		println!("Creating index on collection: {} with URL: {}", collection, url);
		
		let resp = self.auth(self.http.post(url)).json(&index_body).send().await?;
		...
		if !status.is_success() {
			...
			return Err(anyhow!("Ensure index on {} failed: {} - {}", collection, status, txt));
		}
	...
	async fn run_aql<T: for<'de> Deserialize<'de>>(&self, query: &str, bind_vars: Option<serde_json::Value>) -> Result<T> {
	...
	async fn insert_doc(&self, collection: &str, doc: serde_json::Value, dry: bool) -> Result<()> {
		...
		if !status.is_success() {
			let txt = resp.text().await.unwrap_or_default();
			return Err(anyhow!("Insert into {} failed: {} - {}", collection, status, txt));
		}
		Ok(())
	}
	...
	async fn delete_doc(&self, collection: &str, key: &str) -> Result<()> {
		...
		if !status.is_success() {
			let txt = resp.text().await.unwrap_or_default();
			return Err(anyhow!("Delete {}/{} failed: {} - {}", collection, key, status, txt));
		}
		Ok(())
	}
}
...
async fn check_database(client: &Arango, dry: bool) -> Result<()> {
	...
	let required_collections = vec![
		"player", "contest", "game", "venue", 
		"resulted_in", "played_at", "played_with"
	];
...
async fn ensure_meta(client: &Arango, dry: bool) -> Result<()> {
	client.ensure_document_collection("schema_migrations", dry).await?;
	client.ensure_document_collection("migration_lock", dry).await?;
	Ok(())
}
...
async fn acquire_lock(client: &Arango, dry: bool) -> Result<LockGuard> {
	if dry {
		...
		return Ok(LockGuard { client: client.clone() });
	}
			let res = client
		.insert_doc("migration_lock", json!({ "_key": "lock", "acquiredAt": chrono_iso() }), false)
		.await;
	match res {
		Ok(_) => Ok(LockGuard { client: client.clone() }),
...
fn chrono_iso() -> String {
	...
	dt.format(&time::format_description::well_known::Rfc3339).unwrap_or_else(|_| "now".into())
}
...
async fn main() -> Result<()> {
	...
	let client = Arango::authenticate(&args.endpoint, &args.database, &args.username, &args.password).await?;
	...
	for path in files {
		let fname = path.file_name().and_then(|s| s.to_str()).unwrap().to_string();
		...
		match path.extension().and_then(|s| s.to_str()).unwrap_or_default() {
			"json" => {
				...
				apply_migration_file(&client, &mig, args.dry_run).await
			...
			"aql" => {
				...
				if args.dry_run {
					...
					let _: Vec<serde_json::Value> = client.run_aql(&query, None).await
						.with_context(|| format!("AQL {}", fname))?;
				}
			}
			other => {
				return Err(anyhow!("Unsupported migration file extension: {} ({})", other, fname));
			}
		}

		record_applied(&client, &fname, &checksum, start.elapsed().as_millis() as i64, args.dry_run).await?;
...
async fn record_applied(client: &Arango, key: &str, checksum: &str, duration_ms: i64, dry: bool) -> Result<()> {
	let doc = json!({
		"_key": key,
...
async fn apply_migration_file(client: &Arango, mig: &MigrationFile, dry: bool) -> Result<()> {
	for step in &mig.steps {
		match step {
			Step::CreateCollection { name, collection_type, options } => {
				client.create_collection_with_options(name, collection_type.clone(), options.clone(), dry).await?;
			}
			...
			Step::Aql { query, bind_vars } => {
				if dry {
					...
					let _: Vec<serde_json::Value> = client.run_aql(query, bind_vars.clone()).await?;
				}
			}
		}
	}

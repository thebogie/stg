...
**Current workflow:** For up-to-date CI and deployment, see **[CI_CD.md](CI_CD.md)** and **[GHCR_SETUP.md](GHCR_SETUP.md)**. This document is kept for reference; it describes an older flow (e.g. ArangoDB, `setup-hybrid-dev.sh`, `start-frontend.sh`, `.env.development`). The repo now uses SurrealDB, `config/.env.dev`/`.env.prod`, `scripts/start-back.sh`, `scripts/start-front.sh`, and GHCR for production.

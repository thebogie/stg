...
# Helper function to generate JUnit XML from nextest output if XML wasn't auto-generated
generate_junit_xml_from_output() {
    local output_file="$1"
    local xml_file="$2"
    local suite_name="$3"
    
    # Extract test summary from output
    local SUMMARY=$(grep -E "Summary.*tests run" "$output_file" | tail -1 || echo "")
    if [ -n "$SUMMARY" ]; then
        # Parse: "Summary [   8.537s] 466 tests run: 466 passed, 9 skipped"
        local TESTS=$(echo "$SUMMARY" | grep -oE '[0-9]+ tests run' | grep -oE '[0-9]+' || echo "0")
        ...
        local TIME=$(echo "$SUMMARY" | grep -oE '\[[ ]*[0-9.]+s\]' | grep -oE '[0-9.]+' || echo "0")
        
        # Generate basic JUnit XML
        cat > "$xml_file" <<EOF
<?xml version="1.0" encoding="UTF-8"?>
<testsuites>
  <testsuite name="$suite_name" tests="$TESTS" failures="$FAILED" errors="0" skipped="$SKIPPED" time="$TIME">
    <!-- Test results parsed from nextest output -->
  </testsuite>
</testsuites>
EOF
        log_info "Generated JUnit XML with summary: $TESTS tests, $PASSED passed, $FAILED failed, $SKIPPED skipped"
        return 0
    else
        log_warning "Could not parse test summary from output"
        return 1
    fi
}
...
# Try to use --junit-xml flag first, fall back to config file approach
# Run both regular and ignored tests (ignored tests require Redis which is available)
log_info "Command: cargo nextest run --workspace --lib --run-ignored all"
# Capture output to both log file and stdout
TEST_OUTPUT=$(mktemp)
if ! cargo nextest run --workspace --lib --run-ignored all 2>&1 | tee "$TEST_OUTPUT" "$PROJECT_ROOT/_build/test-results/unit-tests-output.log"; then
    log_error "Backend unit tests failed!"
    log_info "Check output: _build/test-results/unit-tests-output.log"
    rm -f "$TEST_OUTPUT"
    exit 1
fi

# Check for XML in multiple possible locations
XML_FOUND=false
if [ -f "$PROJECT_ROOT/_build/test-results.xml" ]; then
    mv "$PROJECT_ROOT/_build/test-results.xml" "$PROJECT_ROOT/_build/test-results/rust-unit-tests.xml"
    log_info "JUnit XML saved to: _build/test-results/rust-unit-tests.xml"
    XML_FOUND=true
elif [ -f "test-results.xml" ]; then
    mv "test-results.xml" "$PROJECT_ROOT/_build/test-results/rust-unit-tests.xml"
    log_info "JUnit XML saved to: _build/test-results/rust-unit-tests.xml"
    XML_FOUND=true
fi

# If XML wasn't generated, try to parse the output and create it manually
if [ "$XML_FOUND" = false ]; then
    log_warning "⚠️  JUnit XML not auto-generated by nextest"
    log_info "Attempting to generate XML from test output..."
    generate_junit_xml_from_output "$TEST_OUTPUT" "$PROJECT_ROOT/_build/test-results/rust-unit-tests.xml" "backend-unit-tests"
fi
rm -f "$TEST_OUTPUT"
...
log_info ""
log_info "Running ALL integration tests in backend/tests/ directory:"
...
log_info "  - ratings_integration_test.rs"
log_info "  - repository_cache_test.rs (including ignored tests - Redis is available)"
log_info "  - database_integration_test.rs (feature-gated, may be skipped)"
mkdir -p "$PROJECT_ROOT/_build/test-results"
# Run ALL integration tests in backend/tests/ directory (including ignored tests)
# Use --test '*' to run all test files, or specify each one explicitly
# --run-ignored all runs both regular and ignored tests
log_info "Command: cargo nextest run --package backend --test '*' --no-fail-fast --run-ignored all"
INTEGRATION_OUTPUT=$(mktemp)
if ! cargo nextest run \
    --package backend \
    --test '*' \
    --no-fail-fast \
    --run-ignored all 2>&1 | tee "$INTEGRATION_OUTPUT" "$PROJECT_ROOT/_build/test-results/integration-tests-output.log"; then
    log_error "E2E-style integration tests failed!"
    log_info "Check output: _build/test-results/integration-tests-output.log"
...
XML_FOUND=false
if [ -f "$PROJECT_ROOT/_build/test-results.xml" ]; then
    mv "$PROJECT_ROOT/_build/test-results.xml" "$PROJECT_ROOT/_build/test-results/rust-integration-tests.xml"
    log_info "JUnit XML saved to: _build/test-results/rust-integration-tests.xml"
    XML_FOUND=true
elif [ -f "test-results.xml" ]; then
    mv "test-results.xml" "$PROJECT_ROOT/_build/test-results/rust-integration-tests.xml"
    log_info "JUnit XML saved to: _build/test-results/rust-integration-tests.xml"
    XML_FOUND=true
fi
...
if [ "$XML_FOUND" = false ]; then
    log_warning "⚠️  JUnit XML not auto-generated by nextest"
    log_info "Attempting to generate XML from test output..."
    generate_junit_xml_from_output "$INTEGRATION_OUTPUT" "$PROJECT_ROOT/_build/test-results/rust-integration-tests.xml" "backend-integration-tests"
fi
rm -f "$INTEGRATION_OUTPUT"
...
mkdir -p "$PROJECT_ROOT/_build/test-results"
# JUnit XML will be generated at _build/test-results.xml, we'll move it after
log_info "Command: cargo nextest run --package backend --test 'cache_integration_test' --no-fail-fast"
CACHE_OUTPUT=$(mktemp)
if ! cargo nextest run \
    --package backend \
    --test 'cache_integration_test' \
    --no-fail-fast 2>&1 | tee "$CACHE_OUTPUT" "$PROJECT_ROOT/_build/test-results/cache-tests-output.log"; then
    ...
    log_info "Check output: _build/test-results/cache-tests-output.log"
...
XML_FOUND=false
if [ -f "$PROJECT_ROOT/_build/test-results.xml" ]; then
    mv "$PROJECT_ROOT/_build/test-results.xml" "$PROJECT_ROOT/_build/test-results/cache-tests.xml"
    log_info "JUnit XML saved to: _build/test-results/cache-tests.xml"
    XML_FOUND=true
elif [ -f "test-results.xml" ]; then
    mv "test-results.xml" "$PROJECT_ROOT/_build/test-results/cache-tests.xml"
    log_info "JUnit XML saved to: _build/test-results/cache-tests.xml"
    XML_FOUND=true
fi
...
if [ "$XML_FOUND" = false ]; then
    log_warning "⚠️  JUnit XML not auto-generated by nextest"
    log_info "Attempting to generate XML from test output..."
    generate_junit_xml_from_output "$CACHE_OUTPUT" "$PROJECT_ROOT/_build/test-results/cache-tests.xml" "backend-cache-tests"
fi
rm -f "$CACHE_OUTPUT"
...
# Ensure test results directory exists for JUnit XML
mkdir -p "$PROJECT_ROOT/_build/test-results"
...
log_info "Checking for test files in testing/e2e/..."
if [ ! -d "testing/e2e" ]; then
    log_error "Test directory 'testing/e2e' not found!"
    exit 1
fi
TEST_COUNT=$(find testing/e2e -name "*.spec.ts" -type f | wc -l)
log_info "Found $TEST_COUNT test file(s) in testing/e2e/"
if [ "$TEST_COUNT" -eq 0 ]; then
    log_warning "⚠️  No test files found in testing/e2e/!"
    log_info "Expected files matching: testing/e2e/*.spec.ts"
fi

if [ "${USE_PRODUCTION_CONTAINERS}" = "1" ]; then
    # Use config-defined reporters (list+junit+html with open:never)
    log_info "Running Playwright with production containers..."
    log_info "Reporters: list, junit, html (open:never)"
    log_info "Command: npx playwright test"
    # Don't override reporters - use config file settings
    npx playwright test 2>&1 | tee /tmp/playwright-output.txt || PLAYWRIGHT_EXIT_CODE=${PIPESTATUS[0]}
else
    # Normal run with HTML reporter
    log_info "Running: npx playwright test"
    npx playwright test 2>&1 | tee /tmp/playwright-output.txt || PLAYWRIGHT_EXIT_CODE=${PIPESTATUS[0]}
fi

PLAYWRIGHT_OUTPUT=$(cat /tmp/playwright-output.txt 2>/dev/null || echo "")
# Save output for debugging
echo "$PLAYWRIGHT_OUTPUT" > "$PROJECT_ROOT/_build/test-results/playwright-output.log" 2>/dev/null || true
rm -f /tmp/playwright-output.txt

# Check if tests were actually discovered and run
TESTS_DISCOVERED=$(echo "$PLAYWRIGHT_OUTPUT" | grep -cE "\[chromium\]|\[Mobile Chrome\]" || echo "0")
TESTS_RUN=$(echo "$PLAYWRIGHT_OUTPUT" | grep -E "Running [0-9]+ test" | grep -oE "[0-9]+" | head -1 || echo "0")
if [ "$TESTS_DISCOVERED" -eq 0 ] && [ "$TESTS_RUN" -eq 0 ]; then
    log_error "⚠️  No tests were discovered or run by Playwright!"
    log_info "This could mean:"
    log_info "  1. Tests failed to connect to frontend"
    log_info "  2. Test files have syntax errors"
    log_info "  3. Playwright configuration issue"
    log_info ""
    log_info "Playwright output (last 100 lines):"
    echo "$PLAYWRIGHT_OUTPUT" | tail -100 | while IFS= read -r line; do
        log_info "  $line"
    done
    log_info ""
    log_info "Full output saved to: _build/test-results/playwright-output.log"
fi

# Extract test summary
TEST_SUMMARY=$(echo "$PLAYWRIGHT_OUTPUT" | grep -E "passed|failed|tests?" | tail -1 || echo "")
if [ -z "$TEST_SUMMARY" ]; then
    log_warning "⚠️  Could not extract test summary from Playwright output"
    log_info "This might indicate tests failed to start or connect to frontend"
    log_info "Check: _build/test-results/playwright-output.log"
fi
...
NON_VISUAL_MATCHES=$(echo "$PLAYWRIGHT_OUTPUT" | grep -iE "Error:" | grep -ivE "toHaveScreenshot|expect\(page\)\.toHaveScreenshot|screenshot|visual.*snapshot" | wc -l | tr -d ' \n' || echo "0")
# Ensure we have a clean integer (remove any newlines or extra whitespace)
VISUAL_MATCHES=$(echo "$VISUAL_MATCHES" | tr -d ' \n')
NON_VISUAL_MATCHES=$(echo "$NON_VISUAL_MATCHES" | tr -d ' \n')
# Default to 0 if empty
VISUAL_MATCHES=${VISUAL_MATCHES:-0}
NON_VISUAL_MATCHES=${NON_VISUAL_MATCHES:-0}
...
# ============================================================================
# STEP 5: Generate Comprehensive Test Summary
# ============================================================================
log_step "STEP 5: Generating Comprehensive Test Summary"
...
# 2. E2E-style Integration Tests
log_info ""
log_info "2. E2E-style Integration Tests:"
if [ -f "_build/test-results/rust-integration-tests.xml" ]; then
    read tests failures errors skipped <<< "$(parse_junit_xml _build/test-results/rust-integration-tests.xml)"
    TOTAL_TESTS=$((TOTAL_TESTS + tests))
    TOTAL_FAILURES=$((TOTAL_FAILURES + failures))
    TOTAL_ERRORS=$((TOTAL_ERRORS + errors))
    TOTAL_SKIPPED=$((TOTAL_SKIPPED + skipped))
    ...
    log_warning "   ⚠️  No test results found: _build/test-results/rust-integration-tests.xml"
fi

# 2.5: Cache Integration Tests
log_info ""
log_info "2.5. Cache Integration Tests:"
if [ -f "_build/test-results/cache-tests.xml" ]; then
    read tests failures errors skipped <<< "$(parse_junit_xml _build/test-results/cache-tests.xml)"
    TOTAL_TESTS=$((TOTAL_TESTS + tests))
    TOTAL_FAILURES=$((TOTAL_FAILURES + failures))
    TOTAL_ERRORS=$((TOTAL_ERRORS + errors))
    TOTAL_SKIPPED=$((TOTAL_SKIPPED + skipped))
    ...
    if [ "$failures" -gt 0 ] || [ "$errors" -gt 0 ]; then
        log_warning "   ⚠️  Failed: $failures failures, $errors errors (non-blocking)"
    else
        log_success "   ✅ Passed: $tests tests"
    fi
    log_info "      Total: $tests | Failures: $failures | Errors: $errors | Skipped: $skipped"
else
    log_warning "   ⚠️  No test results found: _build/test-results/cache-tests.xml"
fi

# 3. Frontend E2E Tests (Playwright)
log_info ""
log_info "3. Frontend E2E Tests (Playwright):"
if [ -f "_build/test-results/e2e-results.xml" ]; then
    ...
    if [ "$failures" -gt 0 ] || [ "$errors" -gt 0 ]; then
        ...
        if grep -qiE "toHaveScreenshot|visual.*snapshot" "_build/test-results/e2e-results.xml" 2>/dev/null; then
            log_warning "   ⚠️  Visual regression failures: $failures (non-blocking)"
        else
            HAS_FAILURES=true
            log_error "   ❌ Failed: $failures failures, $errors errors"
        fi
...
# 4. E2E API Tests (Rust-based)
log_step "4. E2E API Tests (Rust-based, require BACKEND_URL)"
log_info "Running E2E-style API tests against production backend..."
log_info "These tests use BACKEND_URL to connect to the production backend"
log_info "Test files:"
...
if [ -z "${BACKEND_URL:-}" ]; then
    log_error "BACKEND_URL is not set! E2E API tests require BACKEND_URL."
    log_info "These tests will be skipped."
else
    # Verify backend is accessible
    log_info "Verifying backend is accessible at ${BACKEND_URL}..."
    if ! curl -sf "${BACKEND_URL}/health" > /dev/null 2>&1; then
        log_warning "⚠️  Backend health check failed at ${BACKEND_URL}/health"
        log_info "Tests will still run, but may fail if backend is not ready"
    else
    ...
    log_info "Command: cargo nextest run --package testing --test '*_e2e' --no-fail-fast"
    
    E2E_API_OUTPUT=$(mktemp)
    if ! cargo nextest run \
        --package testing \
        --test '*_e2e' \
        --no-fail-fast 2>&1 | tee "$E2E_API_OUTPUT" "$PROJECT_ROOT/_build/test-results/e2e-api-tests-output.log"; then
        log_error "E2E API tests failed!"
        log_info "Check output: _build/test-results/e2e-api-tests-output.log"
        rm -f "$E2E_API_OUTPUT"
    ...
    XML_FOUND=false
    if [ -f "$PROJECT_ROOT/_build/test-results.xml" ]; then
        mv "$PROJECT_ROOT/_build/test-results.xml" "$PROJECT_ROOT/_build/test-results/rust-e2e-api-tests.xml"
        log_info "JUnit XML saved to: _build/test-results/rust-e2e-api-tests.xml"
        XML_FOUND=true
    elif [ -f "test-results.xml" ]; then
        mv "test-results.xml" "$PROJECT_ROOT/_build/test-results/rust-e2e-api-tests.xml"
        log_info "JUnit XML saved to: _build/test-results/rust-e2e-api-tests.xml"
        XML_FOUND=true
    fi
    ...
    if [ "$XML_FOUND" = false ]; then
        log_warning "⚠️  JUnit XML not auto-generated by nextest"
        log_info "Attempting to generate XML from test output..."
        generate_junit_xml_from_output "$E2E_API_OUTPUT" "$PROJECT_ROOT/_build/test-results/rust-e2e-api-tests.xml" "backend-e2e-api-tests"
    fi
    rm -f "$E2E_API_OUTPUT"
    
    # Parse results and add to summary
    if [ -f "$PROJECT_ROOT/_build/test-results/rust-e2e-api-tests.xml" ]; then
        read tests failures errors skipped <<< "$(parse_junit_xml "$PROJECT_ROOT/_build/test-results/rust-e2e-api-tests.xml")"
        TOTAL_TESTS=$((TOTAL_TESTS + tests))
        TOTAL_FAILURES=$((TOTAL_FAILURES + failures))
        TOTAL_ERRORS=$((TOTAL_ERRORS + errors))
        TOTAL_SKIPPED=$((TOTAL_SKIPPED + skipped))
...
if [ "$HAS_FAILURES" = true ]; then
    ...
    log_info "   Check individual test sections above for details"
else

    ...
    C: arangors::client::ClientExt + Send + Sync,
{
    ...
    async fn convert_contests_to_client_dtos(
        ...
        for contest in contests {
            ...
            let participants = self.repository.get_contest_participants(&contest.id).await?;
            ...
            let my_result = my_result.ok_or_else(|| {
                SharedError::NotFound(format!("Current player not found in contest {}", contest.id))
            })?;
    ...
    fn compute_analytics_from_contests(
        ...
        player_id: &str,
    ) -> (ClientStatsDto, Vec<ClientGamePerformanceDto>, Vec<ClientOpponentPerformanceDto>, Vec<ClientTrendDto>) {
        ...
        let stats = ClientStatsDto {
            ...
            total_wins,
            total_losses,
            win_rate: if total_contests > 0 { (total_wins as f64 / total_contests as f64) * 100.0 } else { 0.0 },
            average_placement: if total_contests > 0 { total_placement as f64 / total_contests as f64 } else { 0.0 },
            best_placement: if best_placement == i32::MAX { 0 } else { best_placement },
            worst_placement,
        ...
        for contest in contests {
            ...
            let entry = game_stats.entry(game_id).or_insert_with(|| ClientGamePerformanceDto {
                game: ClientGameDto {
        ...
        game_performance.sort_by(|a, b| b.total_plays.cmp(&a.total_plays).then(b.win_rate.partial_cmp(&a.win_rate).unwrap()));
        ...
        for contest in contests {
            for participant in &contest.participants {
                if participant.player_id != player_id {
                    let entry = opponent_stats.entry(participant.player_id.clone()).or_insert_with(|| ClientHeadToHeadDto {
                        total_contests: 0,
                        my_wins: 0,
                        opponent_wins: 0,
                        my_win_rate: 0.0,
        ...
        opponent_performance.sort_by(|a, b| b.head_to_head.total_contests.cmp(&a.head_to_head.total_contests));
        ...
        let mut trends: Vec<_> = monthly_stats
            ...
            .map(|(period, (contests_played, wins, total_placement))| {
                let win_rate = if contests_played > 0 { (wins as f64 / contests_played as f64) * 100.0 } else { 0.0 };
                let average_placement = if contests_played > 0 { total_placement / contests_played as f64 } else { 0.0 };
    ...
    C: arangors::client::ClientExt + Send + Sync,
{
    async fn sync_client_data(
        ...
        let contests = if request.full_sync {
            self.repository.get_all_contests_for_player(player_id).await?
        } else {
            // Delta sync - get contests since last sync
            let since = request.last_sync.unwrap_or_else(|| Utc::now().fixed_offset());
        ...
        let client_contests = self.convert_contests_to_client_dtos(contests, player_id).await?;
        ...
        let sync_metadata = ClientSyncMetadata {
            total_contests: self.repository.get_total_contests_for_player(player_id).await?,
        ...
        log::info!("Client sync completed for player: {}, contests: {}", player_id, response.contests.len());
        Ok(response)
    }
    ...
    async fn query_client_analytics(
        ...
        let contests = self.repository.get_filtered_contests(player_id, query).await?;
    ...
    async fn validate_client_data(
        ...
        let server_contests = self.repository.get_all_contests_for_player(player_id).await?;
        ...
        let is_valid = server_hash == request.data_hash && server_contest_count == request.contest_count;
        ...
        log::info!("Client data validation completed for player: {}, valid: {}", player_id, is_valid);
        Ok(response)
    }
    ...
    async fn get_client_sync_status(
        ...
        let total_contests = self.repository.get_total_contests_for_player(player_id).await?;
        let _last_contest = self.repository.get_last_contest_for_player(player_id).await?;
    ...
    async fn get_gaming_communities(
        ...
        let communities = self.repository.get_gaming_communities(player_id, min_contests).await?;
        Ok(communities)
    }

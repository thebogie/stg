...
impl BGGService {
    ...
    pub async fn search_games(&self, query: &str) -> Result<Vec<Game>> {
        ...
        let response = self
            .build_request(reqwest::Method::GET, &search_url)
            .query(&params)
            .send()
            .await?;
        ...
        log::info!(
            "BGG search response length: {} characters",
            response_text.len()
        );
        log::info!(
            "BGG search response preview: {}",
        ...
        for item in root.children().filter(|n| n.is_element()) {
            ...
            if name_lower.contains(&query_lower) || query_lower.contains(&name_lower) {
                log::info!(
                    "Parsed BGG game: {} (ID: {}, Year: {:?}) - MATCHES QUERY",
                    name,
                    id,
                    year_published
                );
                ...
                log::debug!(
                    "Skipping BGG game: {} (ID: {}, Year: {:?}) - DOES NOT MATCH QUERY",
                    name,
                    id,
                    year_published
                );
        ...
        log::info!(
            "BGG API returned {} games for query '{}'",
            games.len(),
            query
        );
        Ok(games)
    ...
    pub async fn get_game_details(&self, bgg_id: &str) -> Result<Option<Game>> {
        ...
        let params = [("id", bgg_id), ("stats", "1")];

        let response = self
            .build_request(reqwest::Method::GET, &thing_url)
            .query(&params)
            .send()
            .await?;
    ...
    pub async fn get_popular_games(&self) -> Result<Vec<Game>> {
        ...
        let response = self
            .build_request(reqwest::Method::GET, &hot_url)
            .query(&params)
            .send()
            .await?;

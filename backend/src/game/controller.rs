...
pub async fn get_game_handler_impl<R>(
...
{
    let usecase = GameUseCaseImpl { repo: repo.get_ref().clone() };
    let param = path.into_inner();
    let id = if param.contains('/') { param } else { format!("game/{}", param) };
...
pub async fn get_all_games_handler_impl<R>(
    repo: web::Data<R>,
) -> impl Responder
where
    R: GameRepository + Clone + 'static,
    ...
    R: GameRepository + Clone + 'static,
{
    ...
    let id = if game_id.contains('/') { game_id } else { format!("game/{}", game_id) };

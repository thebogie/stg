...
impl ContestRepositoryImpl {
    pub fn new_with_google_config(db: Database<ReqwestClient>, google_config: Option<(String, String)>) -> Self {
...
pub trait ContestRepository: Send + Sync {
    async fn create_contest(&self, contest_dto: ContestDto, creator_id: String) -> Result<ContestDto, String>;
    ...
    async fn find_contests_by_player_and_game(&self, player_id: &str, game_id: &str) -> Result<Vec<serde_json::Value>, String>;
}

#[async_trait]
impl ContestRepository for ContestRepositoryImpl {
    async fn create_contest(&self, mut contest_dto: ContestDto, creator_id: String) -> Result<ContestDto, String> {
        log::info!("🎯 Starting contest creation process");
        log::info!("🎯 Contest DTO received: name='{}', start='{}', stop='{}'", 
            contest_dto.name, contest_dto.start, contest_dto.stop);
        log::info!("🎯 Contest has {} outcomes and {} games", contest_dto.outcomes.len(), contest_dto.games.len());
        
        // Log the player IDs we're working with
        for (i, outcome) in contest_dto.outcomes.iter().enumerate() {
            log::info!("  Outcome {}: player_id='{}', handle='{}', email='{}'", 
                i, outcome.player_id, outcome.handle, outcome.email);
        }
        
        // Log games
        for (i, game) in contest_dto.games.iter().enumerate() {
            log::info!("  Game {}: id='{}', name='{}', source='{:?}'", 
                i, game.id, game.name, game.source);
        }
        ...
        if contest_dto.name.is_empty() {
            ...
            contest_dto.name = match std::panic::catch_unwind(|| {
                ...
                Ok(empty_name) => {
                    log::warn!("⚠️ generate_contest_name() returned empty string: '{}'", empty_name);
        ...
        let result = match contest_collection.create_document(contest, insert_options).await {
            Ok(result) => {
        ...
        log::info!("✅ Contest document created successfully: id='{}', name='{}'", 
            created_contest.id, created_contest.name);

        // Handle venue based on source and ID
        log::info!("🏢 Processing venue...");
        log::info!("🏢 Venue source: {:?}, ID: '{}'", contest_dto.venue.source, contest_dto.venue.id);
        
        let venue = if contest_dto.venue.source == shared::models::venue::VenueSource::Database && !contest_dto.venue.id.is_empty() {
            log::info!("🏢 Venue is from database, looking up by ID: {}", contest_dto.venue.id);
            ...
            match self.venue_usecase.get_venue(&contest_dto.venue.id).await {
                Ok(venue) => {
                    log::info!("✅ Successfully found existing venue: {} ({})", venue.display_name, venue.id);
                    venue
                },
                Err(e) => {
            ...
            match self.venue_usecase.create_venue(contest_dto.venue.clone()).await {
                Ok(venue) => {
                    log::info!("✅ Successfully created new venue: {} ({})", venue.display_name, venue.id);
                    venue
                },
                Err(e) => {
        ...
        log::info!("🔗 Edge details: contest_id={}, venue_id={}", created_contest.id, venue_id);
        match self.create_played_at_relation(&created_contest.id, &venue_id).await {
        ...
        for (i, game_dto) in contest_dto.games.iter().enumerate() {
            log::info!("🎮 Processing game {}/{}: {} (id: '{}')", i + 1, contest_dto.games.len(), game_dto.name, game_dto.id);
            
            let game = if game_dto.id.starts_with("game/") {
                log::info!("🎮 Game is from database, looking up by ID: {}", game_dto.id);
                // Game is from database, search by ID to get the full game object
                match self.game_usecase.get_game(&game_dto.id).await {
                    Ok(game) => {
                        log::info!("✅ Successfully found existing game: {} ({})", game.name, game.id);
                        game
                    },
                    Err(e) => {
                ...
                match self.game_usecase.create_game(game_dto.clone()).await {
                    Ok(game) => {
                        log::info!("✅ Successfully created new game: {} ({})", game.name, game.id);
                        game
                    },
                    Err(e) => {
            ...
            log::info!("🎮 Game {}/{} processed successfully: {} ({})", i + 1, contest_dto.games.len(), game_name, game_id);
        }

        // Create the played_with relationships for each processed game
        log::info!("🔗 Creating PLAYED_WITH edges...");
        log::info!("🔗 Total PLAYED_WITH edges to create: {}", processed_games.len());
        
        for (i, game) in processed_games.iter().enumerate() {
            log::info!("🔗 Creating PLAYED_WITH edge {}/{}: contest_id={}, game_id={}", i + 1, processed_games.len(), created_contest.id, game.id);
            match self.create_played_with_relation(&created_contest.id, &game.id).await {
                Ok(_) => log::info!("✅ Successfully created PLAYED_WITH edge {}/{}", i + 1, processed_games.len()),
                Err(e) => {
                    log::error!("💥 Failed to create played_with edge {}/{}: {:?}", i + 1, processed_games.len(), e);
        ...
        log::info!("👥 Total outcomes to process: {}", contest_dto.outcomes.len());
        
        let mut processed_outcomes = Vec::new();
        for (i, outcome) in contest_dto.outcomes.iter().enumerate() {
            log::info!("👥 Processing outcome {}/{}: player_id='{}', handle='{}', email='{}'", 
                i + 1, contest_dto.outcomes.len(), outcome.player_id, outcome.handle, outcome.email);
            ...
            let player = if !player_id.is_empty() && is_real_player_id(&player_id) {
                ...
                match self.player_usecase.get_player(&player_id).await {
                    Ok(player) => {
                        log::info!("✅ Found existing player: {} ({})", player.handle, player.id);
                        player
                    },
                    Err(e) => {
                ...
                log::info!("👥 Creating new player from outcome data: handle='{}', email='{}'", outcome.handle, outcome.email);
                ...
                let hashed_password = Argon2::default().hash_password(default_password.as_bytes(), &salt_string)
            ...
            log::info!("👥 Outcome {}/{} processed successfully: player_id='{}', handle='{}'", 
                i + 1, contest_dto.outcomes.len(), final_player_id, outcome.handle);
        }

        // Create the resulted_in relationships for each processed outcome
        log::info!("🔗 Creating RESULTED_IN edges...");
        log::info!("🔗 Total RESULTED_IN edges to create: {}", processed_outcomes.len());
        ...
        for (i, outcome) in processed_outcomes.iter().enumerate() {
            ...
            match self.create_resulted_in_relation(&created_contest.id, outcome).await {
                Ok(_) => log::info!("✅ Successfully created RESULTED_IN edge {}/{}", i + 1, processed_outcomes.len()),
                Err(e) => {
                    log::error!("💥 Failed to create resulted_in edge {}/{}: {:?}", i + 1, processed_outcomes.len(), e);
        ...
        log::info!("📋 Venue DTO created: {} ({})", venue_dto.display_name, venue_dto.id);
    ...
    async fn delete(&self, _id: &str) -> Result<(), String> {
        ...
        log::info!("🔍 Executing AQL query for player {} and game {}", player_id, game_id);
        ...
        match self.db.aql_query::<serde_json::Value>(debug_query).await {
            Ok(debug_results) => {
                log::info!("🔍 Debug: Player has {} total contests", debug_results.len());
        ...
        match self.db.aql_query::<serde_json::Value>(game_debug_query).await {
            Ok(game_results) => {
                log::info!("🔍 Debug: Found {} games with game_id {}", game_results.len(), game_id);
        ...
        match self.db.aql_query::<serde_json::Value>(query).await {
            ...
            Err(e) => {
                log::error!("💥 Failed to query contests for player {} and game {}: {}", player_id, game_id, e);
...
impl ContestRepositoryImpl {
    ...
    async fn create_played_at_relation(&self, contest_id: &str, venue_id: &str) -> Result<(), SharedError> {
        log::info!("🔗 Creating PLAYED_AT edge...");
        log::info!("🔗 Edge details: from='{}', to='{}', label='PLAYED_AT'", contest_id, venue_id);
        ...
        let played_at_collection = match self.db.collection("played_at").await {
            ...
            Err(e) => {
                ...
                return Err(SharedError::Database(format!("Failed to get played_at collection: {}", e)));
            }
        };

        log::info!("🔗 Inserting PLAYED_AT edge document...");
        match played_at_collection.create_document(played_at, InsertOptions::default()).await {
            Ok(result) => {
            ...
            Err(e) => {
                ...
                Err(SharedError::Database(format!("Failed to create played_at relation: {}", e)))
            }
        }
    }

    /// Create a played_with relation between a contest and a game
    async fn create_played_with_relation(&self, contest_id: &str, game_id: &str) -> Result<(), SharedError> {
        log::info!("🔗 Creating PLAYED_WITH edge...");
        log::info!("🔗 Edge details: from='{}', to='{}', label='PLAYED_WITH'", contest_id, game_id);
        ...
        let played_with_collection = match self.db.collection("played_with").await {
            ...
            Err(e) => {
                ...
                return Err(SharedError::Database(format!("Failed to get played_with collection: {}", e)));
            }
        };

        log::info!("🔗 Inserting PLAYED_WITH edge document...");
        match played_with_collection.create_document(played_with, InsertOptions::default()).await {
            Ok(_result) => {
            ...
            Err(e) => {
                ...
                Err(SharedError::Database(format!("Failed to create played_with relation: {}", e)))
            }
        }
    }

    /// Create a resulted_in relation between a contest and a player
    async fn create_resulted_in_relation(&self, contest_id: &str, outcome: &OutcomeDto) -> Result<(), SharedError> {
        log::info!("🔗 Creating RESULTED_IN edge...");
        log::info!("🔗 Edge details: from='{}', to='{}', label='RESULTED_IN', place='{}', result='{}'", 
            contest_id, outcome.player_id, outcome.place, outcome.result);
        ...
        let place = match outcome.place.parse::<i32>() {
            ...
            Err(e) => {
                ...
                return Err(SharedError::Validation(format!("Invalid place value: {}", e)));
            }
        };
        ...
        let resulted_in_collection = match self.db.collection("resulted_in").await {
            ...
            Err(e) => {
                ...
                return Err(SharedError::Database(format!("Failed to get resulted_in collection: {}", e)));
            }
        };

        log::info!("🔗 Inserting RESULTED_IN edge document...");
        match resulted_in_collection.create_document(resulted_in, InsertOptions::default()).await {
            Ok(_result) => {
            ...
            Err(e) => {
                ...
                Err(SharedError::Database(format!("Failed to create resulted_in relation: {}", e)))
            }
        }
    }
}
            ...
            Ok(mut cursor) => {
                    ...
                    match self.db.aql_query::<serde_json::Value>(check_query).await {
                        Ok(check_cursor) => {
                            ...
                            if !check_cursor.is_empty() {
                                log::warn!("❌ Contest exists but missing relationships for ID: {}", id);
                            } else {
                ...
                if let Some(result) = cursor.pop() {
                    ...
                    let id = match contest_data["_id"].as_str() {
                        ...
                        None => {
                            log::error!("❌ Failed to extract contest ID from: {:?}", contest_data["_id"]);
                            return None;
                        }
                    };
                    ...
                    let name = match contest_data["name"].as_str() {
                        ...
                        None => {
                            log::error!("❌ Failed to extract contest name from: {:?}", contest_data["name"]);
                            return None;
                        }
                    };
                    ...
                    let start_str = match contest_data["start"].as_str() {
                        ...
                        None => {
                            log::error!("❌ Failed to extract contest start from: {:?}", contest_data["start"]);
                            return None;
                        }
                    };
                    ...
                    let stop_str = match contest_data["stop"].as_str() {
                        ...
                        None => {
                            log::error!("❌ Failed to extract contest stop from: {:?}", contest_data["stop"]);
                            return None;
                        }
                    };
                    ...
                    let start = match chrono::DateTime::parse_from_rfc3339(start_str) {
                        ...
                        Err(e) => {
                            log::error!("❌ Failed to parse start date '{}' as RFC3339: {}", start_str, e);
                            return None;
                        }
                    };
                    ...
                    let stop = match chrono::DateTime::parse_from_rfc3339(stop_str) {
                        ...
                        Err(e) => {
                            log::error!("❌ Failed to parse stop date '{}' as RFC3339: {}", stop_str, e);
                            return None;
                        }
                    };
                    ...
                    let venue_dto = match (
                        ...
                        (Some(id), Some(display_name), Some(formatted_address), Some(place_id), Some(lat), Some(lng), timezone) => {
                        ...
                        (Some(id), display_name, formatted_address, Some(place_id), Some(lat), Some(lng), timezone) => {
                            // Handle null display_name and formatted_address with defaults
                            let display_name = display_name.unwrap_or("Unknown Venue");
                            let formatted_address = formatted_address.unwrap_or("Address not available");
                        ...
                        (Some(id), display_name, formatted_address, place_id, lat, lng, timezone) => {
...
impl ContestRepositoryImpl {
    ...
    pub(crate) fn build_game_filter_clause(game_ids_full: &Vec<String>) -> Option<String> {
        if game_ids_full.is_empty() { return None; }
    ...
    pub async fn search_contests(
        ...
        let venue_full = venue_id.map(|v| if v.contains('/') { v.to_string() } else { format!("venue/{}", v) });
        let game_full: Vec<String> = game_ids.iter().map(|g| if g.contains('/') { g.clone() } else { format!("game/{}", g) }).collect();
        let player_full = if player_id.is_empty() { None } else { Some(if player_id.contains('/') { player_id.to_string() } else { format!("player/{}", player_id) }) };

        let mut filters = Vec::new();
        if !q.is_empty() { filters.push("LIKE(contest.name, @q, true)".to_string()); }
        if start_from.is_some() { filters.push("contest.start >= DATE_ISO8601(@start_from)".to_string()); }
        if start_to.is_some() { filters.push("contest.start <= DATE_ISO8601(@start_to)".to_string()); }
        if stop_from.is_some() { filters.push("contest.stop >= DATE_ISO8601(@stop_from)".to_string()); }
        if stop_to.is_some() { filters.push("contest.stop <= DATE_ISO8601(@stop_to)".to_string()); }
        if venue_full.is_some() { filters.push("venue_edge != null && venue_edge._to == @venue_id".to_string()); }
        if let Some(game_clause) = Self::build_game_filter_clause(&game_full) { filters.push(game_clause); }
        ...
        let filter_clause = if filters.is_empty() { String::new() } else { format!("FILTER {}", filters.join(" AND ")) };

        let sort_field = match sort_by { "stop" => "contest.stop", "created_at" => "contest._key", _ => "contest.start" };
        let sort_dir = if sort_dir.eq_ignore_ascii_case("asc") { "ASC" } else { "DESC" };
...
"#);
        ...
        let mut bind_vars: std::collections::HashMap<&str, serde_json::Value> = std::collections::HashMap::new();
        if !q.is_empty() {
            bind_vars.insert("q", serde_json::Value::String(format!("%{}%", q)));
        }
        bind_vars.insert("scope", serde_json::Value::String(scope.to_string()));
        bind_vars.insert("skip", serde_json::Value::Number(serde_json::Number::from(skip as i64)));
        bind_vars.insert("limit", serde_json::Value::Number(serde_json::Number::from(page_size as i64)));
        ...
        if !game_full.is_empty() {
            bind_vars.insert("game_ids", serde_json::Value::Array(game_full.iter().cloned().map(serde_json::Value::String).collect()));
        }

        // First, compute the full total without LIMIT using a lightweight COUNT query
        let count_aql = format!(r#"
LET my_player = @player_id
FOR contest IN contest
    LET venue_edge = FIRST(FOR e IN played_at FILTER e._from == contest._id RETURN e)
    LET game_edge = FIRST(FOR e IN played_with FILTER e._from == contest._id RETURN e)
...
"#);
        ...
        let mut count_bind_vars: std::collections::HashMap<&str, serde_json::Value> = std::collections::HashMap::new();
        if !q.is_empty() {
        ...
        if let Some(sf) = start_from { count_bind_vars.insert("start_from", serde_json::Value::String(sf.to_string())); }
        if let Some(st) = start_to { count_bind_vars.insert("start_to", serde_json::Value::String(st.to_string())); }
        if let Some(ef) = stop_from { count_bind_vars.insert("stop_from", serde_json::Value::String(ef.to_string())); }
        if let Some(et) = stop_to { count_bind_vars.insert("stop_to", serde_json::Value::String(et.to_string())); }
        if let Some(ref v) = venue_full { count_bind_vars.insert("venue_id", serde_json::Value::String(v.clone())); }
        if !game_full.is_empty() {
            count_bind_vars.insert("game_ids", serde_json::Value::Array(game_full.iter().cloned().map(serde_json::Value::String).collect()));
        }
        ...
        let count_query = arangors::AqlQuery::builder()
            ...
            .build();
        let count_result = self.db.aql_query::<i64>(count_query).await.map_err(|e| e.to_string())?;
        ...
        let items_query = arangors::AqlQuery::builder()
            ...
            .bind_vars(bind_vars)
            .build();
        let result = self.db.aql_query::<serde_json::Value>(items_query).await.map_err(|e| e.to_string())?;
        log::info!("🔍 Search query returned {} items (page {} of size {}), total {}", result.len(), page, page_size, total);

...
impl<C: ClientExt + Send + Sync + 'static> RatingsScheduler<C> {
    ...
    pub async fn trigger_recalculation(&self, period: Option<String>) -> Result<()> {
        info!("Manually triggering ratings recalculation for period: {:?}", period);
    ...
    fn calculate_next_run_time() -> DateTime<Utc> {
        ...
        if now.day() == 1 && now.hour() >= 2 {
            ...
            if now.month() == 12 {
                ...
                Utc.with_ymd_and_hms(now.year(), now.month() + 1, 1, 2, 0, 0).unwrap()
            }
        } else if now.day() == 1 && now.hour() < 2 {
            // Same day, 2 AM
            Utc.with_ymd_and_hms(now.year(), now.month(), 1, 2, 0, 0).unwrap()
        } else {
...
mod tests {
    ...
    fn test_is_first_of_month_at_2am() {
        ...
        assert!(RatingsScheduler::<arangors::client::reqwest::ReqwestClient>::is_first_of_month_at_2am(dt));
    ...
    fn test_calculate_next_run_time() {
        ...
        let next_run = RatingsScheduler::<arangors::client::reqwest::ReqwestClient>::calculate_next_run_time();

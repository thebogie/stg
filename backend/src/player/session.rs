...
mod tests {
    ...
    async fn test_mock_session_store_get_session() {
        ...
        // Set a session first
        store.set_session("test_session", "test@example.com").await.unwrap();
    ...
    async fn test_mock_session_store_multiple_sessions() {
        ...
        store.set_session("session1", "user1@example.com").await.unwrap();
        store.set_session("session2", "user2@example.com").await.unwrap();
        store.set_session("session3", "user3@example.com").await.unwrap();
        
        // Verify all sessions exist
        assert_eq!(store.get_session("session1").await.unwrap(), Some("user1@example.com".to_string()));
        assert_eq!(store.get_session("session2").await.unwrap(), Some("user2@example.com".to_string()));
        assert_eq!(store.get_session("session3").await.unwrap(), Some("user3@example.com".to_string()));
    ...
    async fn test_mock_session_store_session_overwrite() {
        ...
        // Set initial session
        store.set_session("test_session", "user1@example.com").await.unwrap();
        assert_eq!(store.get_session("test_session").await.unwrap(), Some("user1@example.com".to_string()));
        
        // Overwrite with new email
        store.set_session("test_session", "user2@example.com").await.unwrap();
        assert_eq!(store.get_session("test_session").await.unwrap(), Some("user2@example.com".to_string()));
    }
    ...
    async fn test_mock_session_store_special_characters() {
        ...
        assert_eq!(store.get_session(session_id).await.unwrap(), Some(email.to_string()));
    }
    ...
    async fn test_mock_session_store_concurrent_access() {
        ...
        let handle1 = tokio::spawn(async move {
            store_clone.set_session("session1", "user1@example.com").await.unwrap();
            store_clone.get_session("session1").await.unwrap()
        });
        
        let handle2 = tokio::spawn(async move {
            store_clone2.set_session("session2", "user2@example.com").await.unwrap();
    ...
    async fn test_mock_session_store_cleanup_after_delete() {
        ...
        for i in 1..=10 {
            ...
            assert_eq!(store.get_session(&session_id).await.unwrap(), expected_email);
        }
    }
} 
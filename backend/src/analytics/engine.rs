...
use shared::{dto::analytics::*, models::analytics::*};
...
impl AnalyticsEngine {
    ...
    pub fn calculate_player_stats(
        &self,
        player_id: &str,
        contest_results: Vec<ContestResult>,
    ) -> PlayerStats {
    ...
    pub fn calculate_contest_stats(
        &self,
        contest_id: &str,
        ...
        if !participants.is_empty() {
            let avg_skill: f64 = participants.iter().map(|p| p.skill_rating).sum::<f64>()
                / participants.len() as f64;
            stats.difficulty_rating =
                self.calculate_difficulty_rating(avg_skill, participants.len());
        }
    ...
    pub fn calculate_game_stats(&self, game_id: &str, game_plays: Vec<GamePlay>) -> GameStats {
        ...
        let unique_players: std::collections::HashSet<String> = game_plays
            .iter()
        ...
        if !game_plays.is_empty() {
            let total_duration: f64 = game_plays
                .iter()
                .map(|play| play.duration_minutes as f64)
                .sum();
    ...
    pub fn calculate_venue_stats(
        &self,
        venue_id: &str,
        venue_contests: Vec<VenueContest>,
    ) -> VenueStats {
        ...
        // Count unique players
        let unique_players: std::collections::HashSet<String> = venue_contests
            .iter()
        ...
        if !venue_contests.is_empty() {
            let total_participants: f64 = venue_contests
                .iter()
                .map(|contest| contest.participant_count as f64)
                .sum();
        ...
        // Calculate average duration
        if !venue_contests.is_empty() {
            let total_duration: f64 = venue_contests
                .iter()
                .map(|contest| contest.duration_minutes as f64)
                .sum();
    ...
    pub fn calculate_elo_change(
        ...
        // Clamp rating change
        rating_change
            .max(self.elo_config.min_rating_change)
    ...
    pub fn calculate_popularity_trend(&self, game_plays: &[GamePlay]) -> Vec<MonthlyPlays> {
        ...
        let mut trend: Vec<MonthlyPlays> = monthly_plays
            .into_iter()
    ...
    pub fn calculate_popular_games(&self, venue_contests: &[VenueContest]) -> Vec<GamePopularity> {
        ...
        let mut popular_games: Vec<GamePopularity> = game_stats
            .into_iter()
    ...
    pub fn calculate_monthly_contests(
        &self,
        venue_contests: &[VenueContest],
    ) -> Vec<MonthlyContests> {
        ...
        let mut trend: Vec<MonthlyContests> = monthly_contests
            .into_iter()
            .map(|((year, month), contests)| MonthlyContests {
                year,
                month,
                contests,
            })

use shared::{
    dto::analytics::*,
    models::analytics::AchievementCategory,
    Result,
};
...
impl<C: ClientExt> AnalyticsUseCase<C> {
    ...
    pub async fn get_contest_heatmap(&self, weeks: i32, game_id: Option<&str>) -> Result<serde_json::Value> {
    ...
    pub async fn get_platform_stats(&self) -> Result<PlatformStatsDto> {
        ...
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::platform_stats()).await;
        
        Ok(dto)
    }

    /// Get leaderboard data with caching
    pub async fn get_leaderboard(&self, request: &LeaderboardRequest) -> Result<LeaderboardResponse> {
        ...
        let entries = self.repo.get_leaderboard(category_str, limit, offset).await?;
        ...
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::leaderboard()).await;

        Ok(response)
    }

    /// Get player statistics with caching
    pub async fn get_player_stats(&self, player_id: &str, _request: &PlayerStatsRequest) -> Result<PlayerStatsDto> {
        ...
        // Cache the result
        let json_data = serde_json::to_string(&dto)?;
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::player_stats()).await;
    ...
    pub async fn get_player_achievements(&self, player_id: &str) -> Result<PlayerAchievementsDto> {
        ...
        let achievement_dtos: Vec<AchievementDto> = achievements.achievements.into_iter()
        ...
        // Cache the result
        let json_data = serde_json::to_string(&dto)?;
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::player_achievements()).await;
    ...
    pub async fn get_player_rankings(&self, player_id: &str) -> Result<Vec<PlayerRankingDto>> {
        ...
        let ranking_dtos: Vec<PlayerRankingDto> = rankings.into_iter()
        ...
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::player_rankings()).await;
    ...
    pub async fn get_contest_stats(&self, contest_id: &str) -> Result<ContestStatsDto> {
        ...
        // Cache the result
        let json_data = serde_json::to_string(&dto)?;
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::contest_stats()).await;
    ...
    pub async fn get_contest_trends(&self, months: i32) -> Result<Vec<MonthlyContestsDto>> {
        ...
        let trend_dtos: Vec<MonthlyContestsDto> = trends.into_iter()
        ...
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::contest_trends()).await;
    ...
    pub async fn get_recent_contests(&self, limit: i32) -> Result<Vec<ContestStatsDto>> {
        ...
        let contest_dtos: Vec<ContestStatsDto> = contests.into_iter()
            .map(|c| ContestStatsDto::from(&c))
            .collect();
        
        // Cache the result
        let json_data = serde_json::to_string(&contest_dtos)?;
        self.cache.set_with_ttl(cache_key, json_data, CacheTTL::recent_contests()).await;
    ...
    pub async fn invalidate_player_cache(&self, player_id: &str) {
        self.cache.invalidate_pattern(&format!("player:{}", player_id)).await;
    }

    /// Invalidate cache for a specific contest
    pub async fn invalidate_contest_cache(&self, contest_id: &str) {
        self.cache.invalidate_pattern(&format!("contest:{}", contest_id)).await;
    }
    ...
    pub async fn get_player_performance_chart(&self, limit: i32, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        for player_id in sample_players.iter().take(limit as usize) {
            if let Ok(stats) = self.get_player_stats(player_id, &PlayerStatsRequest {
                player_id: player_id.to_string(),
        ...
        self.visualization.player_performance_trend(&player_stats, config)
    }

    /// Generate leaderboard chart
    pub async fn get_leaderboard_chart(&self, category: &str, limit: i32, config: Option<ChartConfig>) -> Result<Chart> {
    ...
    pub async fn get_achievement_distribution_chart(&self, player_id: &str, config: Option<ChartConfig>) -> Result<Chart> {
        let achievements = self.get_player_achievements(player_id).await?;
        self.visualization.achievement_distribution(&achievements, config)
    }

    /// Generate contest trends chart
    pub async fn get_contest_trends_chart(&self, months: i32, config: Option<ChartConfig>) -> Result<Chart> {
    ...
    pub async fn get_player_comparison_chart(&self, player_ids: &[String], config: Option<ChartConfig>) -> Result<Chart> {
        let mut player_stats = Vec::new();
        ...
        self.visualization.player_comparison_radar(&player_stats, config)
    }

    /// Generate contest analysis scatter plot
    pub async fn get_contest_analysis_chart(&self, limit: i32, config: Option<ChartConfig>) -> Result<Chart> {
        let contests = self.get_recent_contests(limit).await?;
        self.visualization.contest_analysis_scatter(&contests, config)
    }

    /// Generate games by player count distribution chart
    pub async fn get_game_popularity_heatmap(&self, _limit: i32, config: Option<ChartConfig>) -> Result<Chart> {
        // Get real data from repository
        let player_count_data = self.repo.get_games_by_player_count().await?;
        self.visualization.game_popularity_heatmap(&player_count_data, config)
    }
    ...
    pub async fn get_analytics_dashboard(&self, config: Option<ChartConfig>) -> Result<Vec<Chart>> {
        ...
        // Leaderboard chart
        if let Ok(leaderboard_chart) = self.get_leaderboard_chart("win_rate", 10, config.clone()).await {
    ...
    pub async fn get_custom_chart(&self, chart_type: &str, data_type: &str, config: Option<ChartConfig>) -> Result<Chart> {
        match (chart_type, data_type) {
            ...
            ("pie", "achievements") => {
                ...
                self.get_achievement_distribution_chart("player/1", config).await
            },
            ...
            ("radar", "player_comparison") => {
                let player_ids = vec!["player/1".to_string(), "player/2".to_string(), "player/3".to_string()];
                self.get_player_comparison_chart(&player_ids, config).await
            },
            ("line", "activity_metrics") => {
                self.get_activity_metrics_chart(30, config).await
            }
            _ => Err(shared::SharedError::Conversion(format!("Unsupported chart type: {} for data type: {}", chart_type, data_type))),
        }
    }
    ...
    pub async fn get_players_who_beat_me(&self, player_id: &str) -> Result<Vec<PlayerOpponentDto>> {
        ...
        self.cache.set_with_ttl(cache_key, result_json, CacheTTL::player_opponents()).await;
    ...
    pub async fn get_my_game_performance(&self, player_id: &str) -> Result<Vec<GamePerformanceDto>> {
        ...
        self.cache.set_with_ttl(cache_key, result_json, CacheTTL::player_stats()).await;
        
        Ok(performance)
    }

    /// Get player's head-to-head record against specific opponent
    pub async fn get_head_to_head_record(&self, player_id: &str, opponent_id: &str) -> Result<HeadToHeadRecordDto> {
        // Get head-to-head record
        let record = self.repo.get_head_to_head_record(player_id, opponent_id).await?;
        
        // Cache the result
        let cache_key = CacheKeys::head_to_head_record(player_id, opponent_id);
        let result_json = serde_json::to_string(&record)?;
        self.cache.set_with_ttl(cache_key, result_json, CacheTTL::head_to_head()).await;
        
        Ok(record)
    }

    /// Get player's performance trends over time
    pub async fn get_my_performance_trends(&self, player_id: &str) -> Result<Vec<PerformanceTrendDto>> {
...
impl<C: ClientExt> AnalyticsUseCase<C> {
    ...
    pub async fn get_activity_metrics_chart(&self, days: i32, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        for (i, month_label) in month_labels.iter().enumerate() {
            ...
            monthly_players.data.push(crate::analytics::visualization::DataPoint { 
                label: month_label.clone(), 
                value: player_count, 
                color: None, 
                metadata: None 
            });
            monthly_contests.data.push(crate::analytics::visualization::DataPoint { 
                label: month_label.clone(), 
                value: contest_count, 
                color: None, 
                metadata: None 
    ...
    pub async fn get_player_performance_distribution_chart(&self, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        let data_points: Vec<crate::analytics::visualization::DataPoint> = distribution
            .into_iter()
            .map(|(range, count)| crate::analytics::visualization::DataPoint {
                label: range,
                value: count as f64,
                color: None,
                metadata: None,
        ...
        Ok(crate::analytics::visualization::Chart {
            ...
            metadata: std::collections::HashMap::from([
                ("description".to_string(), "Distribution of players by win rate ranges".to_string()),
    ...
    pub async fn get_game_difficulty_popularity_chart(&self, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        let data_points: Vec<crate::analytics::visualization::DataPoint> = games
            .into_iter()
            .map(|(game_name, difficulty, popularity, win_rate)| crate::analytics::visualization::DataPoint {
                label: game_name,
                value: difficulty,
                color: Some(format!("rgba(59, 130, 246, {})", (win_rate / 100.0).max(0.1))),
        ...
        Ok(crate::analytics::visualization::Chart {
            ...
            data: crate::analytics::visualization::ChartData::SingleSeries(data_points),
            metadata: std::collections::HashMap::from([
                ("description".to_string(), "Game difficulty rating vs contest popularity".to_string()),
    ...
    pub async fn get_venue_performance_timeslot_chart(&self, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        for (venue, timeslot, rate) in performance {
            venue_data.entry(venue).or_insert_with(Vec::new).push((timeslot, rate));
        }

        let venues: Vec<String> = venue_data.keys().cloned().collect();
        let timeslots = vec!["Morning".to_string(), "Afternoon".to_string(), "Evening".to_string()];
        
        let mut heatmap_data = Vec::new();
        for venue in &venues {
            let mut row = Vec::new();
            for timeslot in &timeslots {
                let rate = venue_data.get(venue)
        ...
        Ok(crate::analytics::visualization::Chart {
            ...
            metadata: std::collections::HashMap::from([
                ("description".to_string(), "Contest distribution by venue and time slot".to_string()),
    ...
    pub async fn get_player_retention_cohort_chart(&self, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        let data_points: Vec<crate::analytics::visualization::DataPoint> = cohorts
            .into_iter()
            .map(|(contest_num, player_count, retention_rate)| crate::analytics::visualization::DataPoint {
                label: contest_num,
        ...
        Ok(crate::analytics::visualization::Chart {
            ...
            data: crate::analytics::visualization::ChartData::SingleSeries(data_points),
            metadata: std::collections::HashMap::from([
                ("description".to_string(), "Player retention rate by contest number".to_string()),
    ...
    pub async fn get_contest_completion_by_game_chart(&self, config: Option<ChartConfig>) -> Result<Chart> {
        let games = self.repo.get_contest_completion_by_game().await?;
        
        let data_points: Vec<crate::analytics::visualization::DataPoint> = games
            .into_iter()
            .map(|(game_name, total_contests, completion_rate)| crate::analytics::visualization::DataPoint {
                label: game_name,
                value: completion_rate,
                color: None,
                metadata: Some(std::collections::HashMap::from([
                    ("total_contests".to_string(), total_contests.to_string()),
                ])),
            })
            .collect();
        ...
        Ok(crate::analytics::visualization::Chart {
            ...
            data: crate::analytics::visualization::ChartData::SingleSeries(data_points),
            metadata: std::collections::HashMap::from([
                ("description".to_string(), "Percentage of contests completed by game".to_string()),
    ...
    pub async fn get_head_to_head_matrix_chart(&self, limit: i32, config: Option<ChartConfig>) -> Result<Chart> {
        ...
        Ok(crate::analytics::visualization::Chart {
            ...
            data: crate::analytics::visualization::ChartData::HeatmapData(heatmap_data),
            metadata: std::collections::HashMap::from([
                ("description".to_string(), "Win rates between top players".to_string()),

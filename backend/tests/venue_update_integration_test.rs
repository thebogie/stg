...
fn admin_token() -> Option<String> { env::var("ADMIN_BEARER_TOKEN").ok() }
fn user_token() -> Option<String> { env::var("USER_BEARER_TOKEN").ok() }
...
async fn venue_update_requires_auth() {
	...
	let res = reqwest::Client::new().put(&url).json(&body).send().await.expect("request ok");

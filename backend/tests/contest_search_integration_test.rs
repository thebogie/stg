...
fn base_url() -> Result<String> {
	env::var("BACKEND_URL")
		.context("BACKEND_URL environment variable not set. These tests require a running backend server.")
}
...
async fn contests_search_accepts_empty_query_and_returns_json() -> Result<()> {
	...
	let body: ContestSearchResponseDto = res.json().await
		.context("Failed to parse JSON response")?;
	assert!(body.count >= body.items.len() as u64,
		"Count ({}) should be >= items length ({})", body.count, body.items.len());

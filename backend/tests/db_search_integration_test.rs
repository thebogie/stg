...
struct GameDto { id: String, name: String }

#[derive(Debug, serde::Deserialize)]
struct VenueDto { id: String, display_name: String }

#[derive(Debug, serde::Deserialize)]
struct PlayerDto { id: String, handle: String }

fn base_url() -> Option<String> { env::var("BACKEND_URL").ok() }

fn skip() -> bool { base_url().is_none() }

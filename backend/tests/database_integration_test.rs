...
mod database_integration_tests {
    use anyhow::Result;
    use arangors::{client::ClientExt, Connection};
    ...
    async fn test_database_connection() -> Result<()> {
        ...
        let conn = Connection::establish_basic_auth(env.arangodb_url(), "root", "test_password")
            .await
    ...
    async fn test_database_creation_and_deletion() -> Result<()> {
        ...
        let db_list = conn
            ...
            .map_err(|e| anyhow::anyhow!("Failed to list databases: {}", e))?;
        assert!(
            db_list.contains(&test_db_name.to_string()),
            "Database {} should exist in list: {:?}",
            test_db_name,
            db_list
        );
    ...
    async fn test_collection_operations() -> Result<()> {
        ...
        assert!(
            collections.iter().any(|c| c.name == collection_name),
            "Collection {} should exist in list: {:?}",
            collection_name,
    ...
    async fn test_document_operations() -> Result<()> {
        ...
        let inserted_doc = collection
            .create_document(test_doc.clone())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to insert document: {}", e))?;

        assert!(
            inserted_doc._id.is_some(),
            "Inserted document should have an ID"
        );

        // Test document retrieval
        let doc_id = inserted_doc._id.unwrap();
        let retrieved_doc = collection
            .document(&doc_id)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to retrieve document: {}", e))?;

        assert_eq!(
            retrieved_doc["name"], "Test Document",
            "Document name mismatch"
        );
        assert_eq!(retrieved_doc["value"], 42, "Document value mismatch");
        assert_eq!(
            retrieved_doc["active"], true,
            "Document active flag mismatch"
        );
        ...
        collection
            .update_document(&doc_id, update_doc)
            .await
    ...
    async fn test_query_operations() -> Result<()> {
        ...
        let filtered_docs: Vec<serde_json::Value> = db
            .aql_query(filtered_query)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to execute filtered AQL query: {}", e))?;
        assert_eq!(
            filtered_docs.len(),
            2,
            "Should have 2 documents with category A"
        );

        // Test sorted query
        let sorted_query = "FOR doc IN test_query SORT doc.value ASC RETURN doc";
        let sorted_docs: Vec<serde_json::Value> = db
            .aql_query(sorted_query)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to execute sorted AQL query: {}", e))?;
        assert_eq!(sorted_docs.len(), 4, "Should have 4 documents");
        assert_eq!(
            sorted_docs[0]["value"], 10,
            "First document should have value 10"
        );
        assert_eq!(
            sorted_docs[3]["value"], 40,
            "Last document should have value 40"
        );
        ...
        let aggregated: Vec<serde_json::Value> = db
            .aql_query(aggregate_query)
            .await
    ...
    async fn test_transaction_operations() -> Result<()> {
        ...
        db.aql_query(transaction_query)
            .await
        ...
        let count: Vec<u64> = db
            .aql_query(count_query)
            .await
    ...
    async fn test_index_operations() -> Result<()> {
        ...
        assert!(
            indexes
                .iter()
                .any(|idx| idx.fields.contains(&"name".to_string())),
        ...
        assert!(
            ...
            "Indexed query should be fast, took {}ms",
    ...
    async fn test_connection_pooling() -> Result<()> {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                let conn = Connection::establish_basic_auth(&url, "root", "test_password")
                    .await
                    .map_err(|e| format!("Failed to establish connection {}: {}", i, e))?;

                let db = conn.database("_system").await.map_err(|e| {
                    format!(
                        "Failed to access system database from connection {}: {}",
                        i, e
                    )
                })?;
    ...
    async fn test_database_performance() -> Result<()> {
        ...
        assert!(
            ...
            "Bulk insert should complete within 30 seconds, took {}s",
            insert_time.as_secs()
        );
        ...
        assert!(
            ...
            "Query should complete within 5 seconds, took {}ms",
            query_time.as_millis()
        );
    ...
    async fn test_error_handling() -> Result<()> {
        ...
        assert!(
            invalid_db.is_err(),
            "Should fail to access non-existent database"
        );
        ...
        assert!(
            invalid_collection.is_err(),
            "Should fail to access non-existent collection"
        );
    ...
    async fn test_database_cleanup() -> Result<()> {
        ...
        let test_dbs: Vec<&String> = db_list_after
            .iter()

...
/// Fast response for Profile first paint: display + stats only (2 light queries).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProfileSummaryDto {
    pub display_label: Option<String>,
    pub player_stats: PlayerStatsDto,
}

/// Lightweight response for Nemesis tab: opponents only. Fetched in parallel with full bundle for faster first paint.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProfileOpponentsDto {
    pub opponents_who_beat_me: Vec<PlayerOpponentDto>,
    pub opponents_i_beat: Vec<PlayerOpponentDto>,
}

/// Single response for the Profile page: display, stats, achievements, game performance, trends, opponents.
/// Reduces round-trips by loading all tab data in one request (SurrealDB-optimized backend).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProfileBundleDto {
    pub display_label: Option<String>,
    pub player_stats: PlayerStatsDto,
    pub achievements: PlayerAchievementsDto,
    pub game_performance: Vec<GamePerformanceDto>,
    pub performance_trends: Vec<PerformanceTrendDto>,
    pub opponents_who_beat_me: Vec<PlayerOpponentDto>,
    pub opponents_i_beat: Vec<PlayerOpponentDto>,
}
...
impl From<&PlayerAchievements> for PlayerAchievementsDto {
    fn from(achievements: &PlayerAchievements) -> Self {
        Self {
            ...
            player_handle: achievements.player_handle.clone(),

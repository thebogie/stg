...
pub trait ClientStorage: Send + Sync {
    ...
    async fn store_analytics_cache(&self, cache: &ClientAnalyticsCache) -> Result<(), StorageError>;
    
    /// Retrieves analytics cache data
    async fn get_analytics_cache(&self, player_id: &str) -> Result<Option<ClientAnalyticsCache>, StorageError>;
    
    /// Stores raw contest data
    async fn store_contest_data(&self, player_id: &str, contests: &[ClientContest]) -> Result<(), StorageError>;
    
    /// Retrieves raw contest data
    async fn get_contest_data(&self, player_id: &str) -> Result<Option<Vec<ClientContest>>, StorageError>;
    ...
    /// Gets sync metadata
    async fn get_sync_metadata(&self, player_id: &str) -> Result<Option<SyncMetadata>, StorageError>;
...
impl ClientStorage for LocalStorageClient {
    async fn store_analytics_cache(&self, cache: &ClientAnalyticsCache) -> Result<(), StorageError> {
        ...
        if data.len() > self.config.max_cache_size_mb * 1024 * 1024 {
            return Err(StorageError::DataTooLarge(data.len(), self.config.max_cache_size_mb * 1024 * 1024));
        }
    ...
    async fn store_venue_data(&self, venue: &ClientVenue) -> Result<(), StorageError> {
        ...
        let data = serde_json::to_string(venue)
            .map_err(|e| StorageError::Serialization(e.to_string()))?;
...
impl AnalyticsDataManager {
    ...
    pub async fn initialize_player_analytics(&mut self, player_id: &str) -> Result<ClientAnalyticsCache, StorageError> {
        ...
        if let Some(cache) = self.storage.get_analytics_cache(player_id).await? {
            if !cache.needs_refresh(self.config.max_cache_age_hours) {
                self.memory_cache.insert(player_id.to_string(), cache.clone());
                return Ok(cache);
            }
        }
    ...
    pub async fn sync_analytics_data(
        ...
        // Update memory cache
        let mut cache = self.memory_cache.get(player_id).cloned()
        ...
        self.storage.store_contest_data(player_id, &contests).await?;

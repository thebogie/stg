...
impl ClientAnalyticsCache {
    ...
    pub fn build_lookups(&mut self) {
        ...
        for contest in &self.contests {
            ...
            if !self.game_lookup.contains_key(&contest.game.id) {
                self.game_lookup.insert(contest.game.id.clone(), contest.game.clone());
            }
            
            // Venue lookup
            if !self.venue_lookup.contains_key(&contest.venue.id) {
                self.venue_lookup.insert(contest.venue.id.clone(), contest.venue.clone());
            }
            
            // Opponent lookup
            for participant in &contest.participants {
                if participant.player_id != self.player_id && !self.opponent_lookup.contains_key(&participant.player_id) {
                    ...
                    self.opponent_lookup.insert(participant.player_id.clone(), opponent);
                }
            }
        }
    ...
    fn compute_opponent_stats(&mut self) {
        for opponent in self.opponent_lookup.values_mut() {
            ...
            for contest in &self.contests {
                if contest.participants.iter().any(|p| p.player_id == opponent.player_id) {
                    contests_against += 1;
            ...
            opponent.win_rate_against = if contests_against > 0 { (wins_against as f64 / contests_against as f64) * 100.0 } else { 0.0 };
    ...
    pub fn query_analytics(&self, query: AnalyticsQuery) -> ComputedAnalytics {
        ...
        if let Some(opponents) = &query.opponents {
            filtered_contests.retain(|c| {
                c.participants.iter().any(|p| opponents.contains(&p.player_id))
            });
        }
    ...
    fn compute_game_performance(&self, contests: &[ClientContest]) -> Vec<GamePerformance> {
        ...
        for contest in contests {
            ...
            let entry = game_stats.entry(game_id).or_insert_with(|| GamePerformance {
        ...
        for performance in game_stats.values_mut() {
            ...
            if let Some((favorite_venue_id, _)) = venue_counts.iter().max_by_key(|(_, &count)| count) {
        ...
        result.sort_by(|a, b| b.total_plays.cmp(&a.total_plays).then(b.win_rate.partial_cmp(&a.win_rate).unwrap()));
        result
    }
    ...
    fn compute_opponent_performance(&self, contests: &[ClientContest]) -> Vec<OpponentPerformance> {
        ...
        for contest in contests {
            for participant in &contest.participants {
                if participant.player_id != self.player_id {
                    let entry = opponent_stats.entry(participant.player_id.clone()).or_insert_with(|| HeadToHeadStats {
                        total_contests: 0,
                        my_wins: 0,
                        opponent_wins: 0,
                        my_win_rate: 0.0,
        ...
        let mut result: Vec<_> = opponent_stats
            ...
            .filter_map(|(opponent_id, stats)| {
                self.opponent_lookup.get(&opponent_id).map(|opponent| OpponentPerformance {
        ...
        result.sort_by(|a, b| b.head_to_head.total_contests.cmp(&a.head_to_head.total_contests));
        result
    }

...
impl AnalyticsController {
    ...
    /// Creates a new analytics controller with the given cache (e.g. Redis for shared/persistent cache).
    pub fn new_with_cache(
        db: Db,
        config: DatabaseConfig,
        cache: crate::analytics::cache::AnalyticsCacheKind,
    ) -> Self {
        let repo = AnalyticsRepository::new(db, config);
    ...
    /// Get player achievements. Resolve "me" to current user's player id like profile summary.
    pub async fn get_player_achievements(
        ...
        let player_id = if player_param.eq_ignore_ascii_case("me") {
            let email = match req.extensions().get::<String>() {
                Some(e) => e.clone(),
                None => {
    ...
    /// Get full profile bundle for the Profile page (display, stats, achievements, game performance, trends, opponents).
    /// One request replaces many; backend runs SurrealDB queries in parallel.
    /// If path player_id is "me", resolve the current user's player_id from session email (ensures correct SurrealDB id).
    ...
    /// Get opponents only (nemesis data) for fast Nemesis tab. Resolve "me" like get_profile_bundle.
    pub async fn get_profile_opponents(
        &self,
        req: HttpRequest,
        path: web::Path<String>,
    ) -> Result<HttpResponse, actix_web::Error> {
        let player_param = path.into_inner().replace('`', "");
        log::info!("Nemesis tab: loading opponents for player {}", player_param);
        let player_id = if player_param.eq_ignore_ascii_case("me") {
            let email = match req.extensions().get::<String>() {
                Some(e) => e.clone(),
                None => {
        ...
        match self.usecase.get_profile_opponents(&player_id).await {
            Ok(opponents) => {
                log::info!(
                    "Nemesis tab: returning {} who beat me, {} I beat",
                    opponents.opponents_who_beat_me.len(),
                    opponents.opponents_i_beat.len()
                );
                Ok(HttpResponse::Ok().json(opponents))
            }
            Err(e) => {
                log::error!("Failed to get profile opponents for {}: {:?}", player_id, e);
                Ok(HttpResponse::InternalServerError().json(json!({
                    "error": "Failed to load opponents"
                })))
            }
        }
    }

    /// Fast profile summary (display + stats only) for first paint. Resolve "me" like get_profile_bundle.
    /// For "me" we use the exact record id (Thing) from the DB so stats query matches.
    pub async fn get_profile_summary(
        ...
        let player_param = path.into_inner().replace('`', "");
        let (player_id, player_thing, player_email) = if player_param.eq_ignore_ascii_case("me") {
            let email = match req.extensions().get::<String>() {
                Some(e) => e.clone(),
                None => {
                    return Ok(HttpResponse::Unauthorized().json(json!({
            ...
            // Standard SurrealDB pattern: resolve player_id for display/cache, pass email for stats (one query by email, no id binding)
            match self.usecase.repo().get_player_id_by_email(&email).await {
                Ok(Some(pid)) => (pid, None, Some(email)),
                Ok(None) => {
                    return Ok(HttpResponse::NotFound().json(json!({
                        "error": "Player not found for current user"
                    })));
...
/// Configure analytics routes. Pass prefix "/api" for /api/analytics, "" for /analytics (Trunk proxy strips /api).
pub fn configure_routes(
    ...
    let scope_path = if prefix.is_empty() {
        ...
        format!("{}/analytics", prefix)
    };

    log::debug!("Registering analytics routes at {}:", scope_path);
    ...
    cfg.service(
        web::scope(&scope_path)
            ...
            .route("/health", web::get().to(|req: HttpRequest, controller: web::Data<AnalyticsController>| async move {
                controller.health_check(req).await
            }))
            .route("/test-game-performance", web::get().to(|req: HttpRequest, controller: web::Data<AnalyticsController>| async move {
                controller.test_game_performance_query(req).await
            }))
            .route("/debug", web::get().to(|req: HttpRequest, controller: web::Data<AnalyticsController>| async move {
                controller.debug_database(req).await
            }))
            .route("/platform", web::get().to(|req: HttpRequest, controller: web::Data<AnalyticsController>| async move {
            ...
            .route("/leaderboard", web::get().to(|req: HttpRequest, query: web::Query<LeaderboardRequest>, controller: web::Data<AnalyticsController>| async move {
                controller.get_leaderboard(req, query).await
            }))
            .service(
                web::scope("/players")
                    .wrap(AuthMiddleware { redis: std::sync::Arc::new((*redis_client).clone()) })
                    .route("/{player_id}/stats", web::get().to(|req: HttpRequest, path: web::Path<String>, query: web::Query<PlayerStatsRequest>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_stats(req, path, query).await
                    }))
                    .route("/{player_id}/achievements", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_achievements(req, path).await
                    }))
                    .route("/{player_id}/rankings", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_rankings(req, path).await
                    }))
                    .route("/{player_id}/display", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_display(req, path).await
                    }))
                    .route("/{player_id}/profile", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_profile_bundle(req, path).await
                    }))
                    .route("/{player_id}/profile/summary", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_profile_summary(req, path).await
                    }))
            ...
            .service(
                web::scope("/contests")
                    .route("/{contest_id}/stats", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_stats(req, path).await
                    }))
                    .route("/{contest_id}/difficulty", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_difficulty(req, path).await
                    }))
                    .route("/{contest_id}/excitement", web::get().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_excitement(req, path).await
                    }))
                    .route("/trends", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_trends(req, query).await
                    }))
                    .route("/heatmap", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_heatmap(req, query).await
                    }))
                    .route("/recent", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_recent_contests(req, query).await
                    }))
                    .route("/cache/stats", web::get().to(|req: HttpRequest, controller: web::Data<AnalyticsController>| async move {
                        controller.get_cache_stats(req).await
                    }))
                    .route("/cache/invalidate/player/{player_id}", web::post().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.invalidate_player_cache(req, path).await
                    }))
                    .route("/cache/invalidate/contest/{contest_id}", web::post().to(|req: HttpRequest, path: web::Path<String>, controller: web::Data<AnalyticsController>| async move {
                        controller.invalidate_contest_cache(req, path).await
                    }))
                    .route("/cache/invalidate/all", web::post().to(|req: HttpRequest, controller: web::Data<AnalyticsController>| async move {
                        controller.invalidate_all_cache(req).await
                    }))
            )
            .service(
                web::scope("/charts")
                    .route("/player-performance", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_performance_chart(req, query).await
                    }))
                    .route("/leaderboard", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_leaderboard_chart(req, query).await
                    }))
                    .route("/achievement-distribution/{player_id}", web::get().to(|req: HttpRequest, path: web::Path<String>, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_achievement_distribution_chart(req, path, query).await
                    }))
                    .route("/contest-trends", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_trends_chart(req, query).await
                    }))
                    .route("/activity-metrics", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_activity_metrics_chart(req, query).await
                    }))
                    .route("/platform-dashboard", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_platform_dashboard(req, query).await
                    }))
                    .route("/player-comparison", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_comparison_chart(req, query).await
                    }))
                    .route("/contest-analysis", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_analysis_chart(req, query).await
                    }))
                    .route("/game-popularity", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_game_popularity_heatmap(req, query).await
                    }))
                    .route("/player-performance-distribution", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_performance_distribution_chart(req, query).await
                    }))
                    .route("/game-difficulty-popularity", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_game_difficulty_popularity_chart(req, query).await
                    }))
                    .route("/venue-performance-timeslot", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_venue_performance_timeslot_chart(req, query).await
                    }))
                    .route("/player-retention-cohort", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_player_retention_cohort_chart(req, query).await
                    }))
                    .route("/contest-completion-by-game", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_contest_completion_by_game_chart(req, query).await
                    }))
                    .route("/head-to-head-matrix", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_head_to_head_matrix_chart(req, query).await
                    }))
                    .route("/analytics-dashboard", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_analytics_dashboard(req, query).await
                    }))
                    .route("/custom", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
            ...
            .service(
                web::scope("/player")
                    .wrap(AuthMiddleware { redis: std::sync::Arc::new((*redis_client).clone()) })
                    .route("/opponents-who-beat-me", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_players_who_beat_me(req, query).await
                    }))
                    .route("/opponents-i-beat", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_players_i_beat(req, query).await
                    }))
                    .route("/game-performance", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_my_game_performance(req, query).await
                    }))
                    .route("/performance-trends", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_my_performance_trends(req, query).await
                    }))
                    // Use a greedy matcher to allow slashes in opponent_id (e.g., "player/...")
                    .route("/head-to-head/{opponent_id:.*}", web::get().to(|path: web::Path<String>, req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {
                        controller.get_head_to_head_record(path, req, query).await
                    }))
                    .route("/contests-by-venue", web::get().to(|req: HttpRequest, query: web::Query<std::collections::HashMap<String, String>>, controller: web::Data<AnalyticsController>| async move {

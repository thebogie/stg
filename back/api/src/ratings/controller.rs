...
#[derive(Clone)]
pub struct RatingsController {
    usecase: RatingsUsecase,
    scheduler: web::Data<RatingsScheduler>,
}

impl RatingsController {
    pub fn new(usecase: RatingsUsecase, scheduler: RatingsScheduler) -> Self {
        Self {
            usecase,
    ...
    pub fn configure_routes(
        ...
        cfg.service(
            web::scope(&scope_path)
                ...
                .route("/leaderboard/simple", web::get().to(|_req: HttpRequest, query: web::Query<LeaderboardQuery>, ctrl: web::Data<RatingsController>| async move {
                    let scope = match query.scope.as_deref() { Some("global") | None => RatingScope::Global, Some(s) if s.starts_with("game/") => RatingScope::Game(s.to_string()), _ => RatingScope::Global };
                    let min_games = query.min_games.unwrap_or(10);
                    let limit = query.limit.unwrap_or(50);
                ...
                .route("/leaderboard/enhanced", web::get().to(|_req: HttpRequest, query: web::Query<LeaderboardQuery>, ctrl: web::Data<RatingsController>| async move {
                    let scope = match query.scope.as_deref() { Some("global") | None => RatingScope::Global, Some(s) if s.starts_with("game/") => RatingScope::Game(s.to_string()), _ => RatingScope::Global };
                    let min_games = query.min_games.unwrap_or(10);
                    let limit = query.limit.unwrap_or(50);
                ...
                .route("/debug/player-ids", web::get().to(|_req: HttpRequest, ctrl: web::Data<RatingsController>| async move {
                    match ctrl.usecase.debug_player_ids().await {
                        Ok(debug_info) => Ok::<HttpResponse, actix_web::Error>(HttpResponse::Ok().json(debug_info)),
                        Err(e) => Ok(HttpResponse::InternalServerError().json(serde_json::json!({"error": e.to_string()})))
                    }
                }))
                .route("/debug/player-period", web::get().to(|_req: HttpRequest, query: web::Query<DebugPlayerPeriodQuery>, ctrl: web::Data<RatingsController>| async move {
                ...
                .route("/debug/collections", web::get().to(|_req: HttpRequest, ctrl: web::Data<RatingsController>| async move {
                    match ctrl.usecase.debug_collections().await {
                        Ok(debug_info) => Ok::<HttpResponse, actix_web::Error>(HttpResponse::Ok().json(debug_info)),
                        Err(e) => Ok(HttpResponse::InternalServerError().json(serde_json::json!({"error": e.to_string()})))
                    }
                }))
                .route("/debug/resulted-in-vs-players", web::get().to(|_req: HttpRequest, ctrl: web::Data<RatingsController>| async move {
                    match ctrl.usecase.debug_resulted_in_vs_players().await {
                        Ok(debug_info) => Ok::<HttpResponse, actix_web::Error>(HttpResponse::Ok().json(debug_info)),
                        Err(e) => Ok(HttpResponse::InternalServerError().json(serde_json::json!({"error": e.to_string()})))
                    }
                }))
                .route("/current", web::get().to(|req: HttpRequest, ctrl: web::Data<RatingsController>| async move {
                ...
                .route("/history", web::get().to(|req: HttpRequest, query: web::Query<HistoryQuery>, ctrl: web::Data<RatingsController>| async move {
                ...
                .route("/player/{player_id}", web::get().to(|path: web::Path<String>, _req: HttpRequest, ctrl: web::Data<RatingsController>| async move {
                    fn normalize_id(collection: &str, key_or_id: &str) -> String {
...
mod tests {
    ...
    fn normalize_player_key_to_id() {
        let id = RatingsController::normalize_id("player", "p1");
        assert_eq!(id, "player/p1");
    }

    #[test]
    fn normalize_player_full_id_kept() {
        let id = RatingsController::normalize_id("player", "player/p1");
        assert_eq!(id, "player/p1");
    }

...
impl RatingsUsecase {
    pub fn new(repo: RatingsRepository) -> Self {
        Self {
            repo,
    ...
    pub async fn recompute_month_with_history(&self, period: Option<String>) -> Result<()> {
        ...
        let (year, month) = if let Some(p) = period {
            ...
            if parts.len() != 2 {
                return Err(SharedError::BadRequest(format!("Invalid period: {}", p)));
            }
            (
                parts[0]
                    .parse::<i32>()
                    .map_err(|_| SharedError::BadRequest("invalid year".into()))?,
                parts[1]
                    .parse::<u32>()
                    .map_err(|_| SharedError::BadRequest("invalid month".into()))?,
            )
        } else {
            let now = Utc::now();
            if now.month() == 1 {
        ...
        if contests.is_empty() {
            ...
            log::info!(
                "Processing {} contests for period {}-{:02}",
                contests.len(),
                year,
                month
            );
        ...
        let mut latest: std::collections::HashMap<(String, RatingScope), RatingState> =
        ...
        for c in contests.iter() {
            let cid = c
                ...
                .or_else(|| c.get("_id"))
                .and_then(|v| v.as_str())
                .ok_or(SharedError::Database("contest missing id".into()))?;
            let cid = cid.replace("contest:", "contest/");
        ...
        if let Ok(existing_latest_players) =
            self.repo.get_all_latest_player_ids("global", None).await
        {
        ...
        for player_id in &all_players {
            if let Some(existing_rating) = self
                .repo
                .get_latest_rating("global", None, player_id)
                .await?
            {
                let rating = existing_rating
                    .get("rating")
                    .and_then(|x| x.as_f64())
                    .unwrap_or(self.params.default_rating);
                let rd = existing_rating
                    .get("rd")
                    .and_then(|x| x.as_f64())
                    .unwrap_or(self.params.default_rd);
                let volatility = existing_rating
                    .get("volatility")
                    .and_then(|x| x.as_f64())
                ...
                latest.insert(
                    (player_id.clone(), RatingScope::Global),
                    RatingState {
                        rating,
                        rd,
                        vol: volatility,
        ...
        let mut samples_by_player: std::collections::HashMap<String, Vec<OpponentSample>> =
            std::collections::HashMap::new();
        let mut games_played: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
        let mut wins_by_player: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
        let mut losses_by_player: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
            ...
            for (player_id, player_place) in &contest_results {
                *games_played.entry(player_id.clone()).or_insert(0) += 1;
                ...
                for (opponent_id, opponent_place) in &contest_results {
                    ...
                    let opp_state = *latest
                        .entry((opponent_id.clone(), RatingScope::Global))
        ...
        for player in all_players.into_iter() {
            ...
            let updated = if samples.is_empty() {
                ...
                RatingState {
                    rd: inflated.rd.min(self.params.default_rd),
                    ..inflated
                }
            } else {
    ...
    pub async fn recompute_month(&self, period: Option<String>) -> Result<()> {
        // `recompute_month_with_history` is the correct monthly update implementation:
        // it loads prior (rating, RD, volatility) state and applies inactivity RD inflation.
        self.recompute_month_with_history(period).await
    }
    ...
    pub async fn get_leaderboard(
        ...
        min_games: i32,
        limit: i32,
    ) -> Result<Vec<RatingLeaderboardEntryDto>> {
        ...
        let rows = self
            .repo
            .get_leaderboard(scope_type, scope_id_opt, min_games, limit)
            .await?;
    ...
    pub async fn get_player_rating_history(
        ...
        player_id: &str,
        scope: RatingScope,
    ) -> Result<Vec<PlayerRatingHistoryPointDto>> {
        let (scope_type, scope_id_opt) = match scope {
            RatingScope::Global => ("global", None),
            RatingScope::Game(ref gid) => ("game", Some(gid.as_str())),
        };
        let rows = self
            .repo
            .get_rating_history(player_id, scope_type, scope_id_opt, 180)
            .await?;
        let mut out = Vec::new();
        for v in rows.into_iter() {
    ...
    pub async fn debug_player_period_activity(&self, email: &str, period: &str) -> Result<Value> {
        ...
        if parts.len() != 2 {
            return Err(SharedError::BadRequest(format!(
                "Invalid period: {}",
                period
            )));
        }
        ...
        let current_state = if let Some(existing_rating) = self
            .repo
            .get_latest_rating("global", None, &player_id)
            .await?
        {
        ...
        for c in contests.iter() {
            ...
            for (opp_id, opp_place) in &contest_results {
                ...
                let (opp_rating, opp_rd) =
                    if let Some(r) = self.repo.get_latest_rating("global", None, opp_id).await? {
                        let r0 = r
                            .get("rating")
                            .and_then(|x| x.as_f64())
                            .unwrap_or(self.params.default_rating);
        ...
        let after = if samples.is_empty() {
            ...
            RatingState {
                ...
                rd: (before.rd.powi(2) + before.vol.powi(2))
                    .sqrt()
                    .min(self.params.default_rd),
                vol: before.vol,
            }
        } else {
        ...
        if !samples.is_empty() {
            ...
            let g = |phi_j: f64| {
                1.0 / (1.0 + 3.0 * phi_j.powi(2) / std::f64::consts::PI.powi(2)).sqrt()
            };
        ...
        // Opponent RD stats
        let opp_rd_stats = if opponent_rds.is_empty() {
            None
        } else {
    ...
    pub async fn get_simple_leaderboard(
        ...
        self.repo
            .get_simple_leaderboard(scope_type, scope_id_opt, min_games, limit)
            .await
    }
    ...
    pub async fn get_leaderboard_with_contest_data(
        ...
        self.repo
            .get_leaderboard_with_contest_data(scope_type, scope_id_opt, min_games, limit)
            .await
    }

    /// Get player ID by email
    pub async fn get_player_id_by_email(&self, email: &str) -> Result<Option<String>> {
        self.repo.get_player_id_by_email(email).await
    }
}

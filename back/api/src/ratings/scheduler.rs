...
#[derive(Clone)]
pub struct RatingsScheduler {
    usecase: Arc<RatingsUsecase>,
    last_run: Arc<Mutex<Option<DateTime<Utc>>>>,
    is_running: bool,
}

impl RatingsScheduler {
    pub fn new(usecase: RatingsUsecase) -> Self {
        Self {
    ...
    pub fn is_first_of_month_at_2am(dt: DateTime<Utc>) -> bool {
        dt.day() == 1 && dt.hour() == 2 && dt.minute() < 60
    }

    /// Run the monthly recalculation
    async fn run_monthly_recalculation(usecase: &RatingsUsecase) -> Result<()> {
    ...
    pub fn calculate_next_run_time() -> DateTime<Utc> {
        let now = Utc::now();
...
mod tests {
    ...
    fn test_is_first_of_month_at_2am() {
        ...
        assert!(RatingsScheduler::is_first_of_month_at_2am(dt));
    ...
    fn test_calculate_next_run_time() {
        ...
        let next_run = RatingsScheduler::calculate_next_run_time();

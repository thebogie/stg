                        ...
                        if let Some(result) = json.get("result") {
                            if let Some(geometry) = result.get("geometry") {
                                if let (Some(lat), Some(lng)) = (
                                    ...
                                    if let Some(utc_offset) =
                                        result.get("utc_offset").and_then(|v| v.as_i64())
                                    {
                                        let tz_name = utc_offset_to_timezone(utc_offset);
                                        log::info!(
                                            "Fallback from geometry: utc_offset {} -> {}",
                                            utc_offset,
                                            tz_name
                                        );
                                        return tz_name;
                    ...
                    Err(e) => {
                        ...
                        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&response_text)
                        {
                            ...
                            if let Some(tz_id) = json.get("time_zone_id").and_then(|v| v.as_str()) {
                                log::info!(
                                    "Extracted timezone from raw response (snake_case): {}",
                                    tz_id
                                );
                                return tz_id.to_string();
                            }
...
mod tests {
    ...
    async fn test_timezone_inference_international() {
        let _service = GoogleTimezoneService::new(
            "https://maps.googleapis.com/maps/api/timezone/json".to_string(),
            "test_key".to_string(),
        );
    ...
    fn test_utc_offset_to_timezone_mapping() {
        ...
        assert!(
            result_300.contains("America"),
            "Expected America timezone for -5 hours, got: {}",
            result_300
        );

        let result_240 = utc_offset_to_timezone(-240); // -4 hours
        assert!(
            result_240.contains("America"),
            "Expected America timezone for -4 hours, got: {}",
            result_240
        );

        let result_360 = utc_offset_to_timezone(-360); // -6 hours
        assert!(
            result_360.contains("America"),
            "Expected America timezone for -6 hours, got: {}",
            result_360
        );

        let result_420 = utc_offset_to_timezone(-420); // -7 hours
        assert!(
            result_420.contains("America"),
            "Expected America timezone for -7 hours, got: {}",
            result_420
        );

        let result_480 = utc_offset_to_timezone(-480); // -8 hours
        assert!(
            result_480.contains("America"),
            "Expected America timezone for -8 hours, got: {}",
            result_480
        );
        ...
        assert!(
            result_60.contains("Europe") || result_60.contains("Africa"),
            "Expected Europe/Africa timezone for +1 hour, got: {}",
            result_60
        );

        let result_120 = utc_offset_to_timezone(120); // +2 hours
        assert!(
            result_120.contains("Europe") || result_120.contains("Africa"),
            "Expected Europe/Africa timezone for +2 hours, got: {}",
            result_120
        );

        let result_540 = utc_offset_to_timezone(540); // +9 hours
        assert!(
            result_540.contains("Asia"),
            "Expected Asia timezone for +9 hours, got: {}",
            result_540
        );

        let result_600 = utc_offset_to_timezone(600); // +10 hours
        assert!(
            result_600.contains("Australia") || result_600.contains("Asia"),
            "Expected Australia/Asia timezone for +10 hours, got: {}",
            result_600
        );
    }
    ...
    fn test_utc_offset_to_timezone_edge_cases() {
        ...
        assert!(
            result_720_neg.contains("Pacific"),
            "Expected Pacific timezone for -12 hours, got: {}",
            result_720_neg
        );

        let result_720_pos = utc_offset_to_timezone(720); // +12 hours
        assert!(
            result_720_pos.contains("Pacific"),
            "Expected Pacific timezone for +12 hours, got: {}",
            result_720_pos
        );
        ...
        assert!(
            result_330_neg.contains("America"),
            "Expected America timezone for -5.5 hours, got: {}",
            result_330_neg
        );

        let result_330_pos = utc_offset_to_timezone(330); // +5.5 hours
        assert!(
            result_330_pos.contains("Asia"),
            "Expected Asia timezone for +5.5 hours, got: {}",
            result_330_pos
        );
    ...
    async fn test_place_id_timezone_resolution() {
        ...
        let result = service
            .infer_timezone_from_place_id("ChIJN1t_tDeuEmsRUsoyG83frY4")
            .await;
    ...
    fn test_timezone_candidate_selection() {
        ...
        assert!(
            east_coast_result.contains("America"),
            "Expected America timezone for -4 hours, got: {}",
            east_coast_result
        );
        ...
        assert!(
            west_coast_result.contains("America"),
            "Expected America timezone for -7 hours, got: {}",
            west_coast_result
        );
        ...
        assert!(
            europe_result.contains("Europe") || europe_result.contains("Africa"),
            "Expected Europe/Africa timezone for +1 hour, got: {}",
            europe_result
        );
        ...
        assert!(
            australia_result.contains("Australia") || australia_result.contains("Asia"),
            "Expected Australia/Asia timezone for +10 hours, got: {}",
            australia_result
        );
    }

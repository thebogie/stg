...
pub struct DatabaseConfig {
    /// SurrealDB endpoint URL (e.g. http://localhost:50001)
    pub url: String,
    /// SurrealDB namespace
    pub ns: String,
...
impl Config {
    ...
    /// Prefer primary env var; in Development, fall back to legacy name for backward compatibility.
    fn db_env_opt(primary: &str, legacy: &str) -> Option<String> {
    ...
    fn load_database_config(env: &Environment) -> DatabaseConfig {
        match env {
            Environment::Development => {
                let surreal_url = Self::db_env_opt("SURREAL_URL", "ARANGO_URL");
                match &surreal_url {
                    Some(url) => info!("Found SURREAL_URL in environment: {}", url),
                    None => warn!("SURREAL_URL not found in environment, using default"),
                }
                ...
                if let Some(port) = env::var("SURREALDB_PORT").ok().and_then(|p| p.parse::<u16>().ok()) {
                    if let Ok(parsed) = url::Url::parse(&url) {
                        ...
                        if host != "127.0.0.1" && host != "localhost" {
                            ...
                            info!("Using local SurrealDB URL: {}", url);
                        }
                    }
                }
            ...
            Environment::Production => {
                let db_name = env::var("SURREAL_DB").expect("SURREAL_DB must be set in production");
...
mod tests {
    ...
    fn test_production_config_validation() {
        ...
        let config = Config {
            ...
            database: DatabaseConfig {
                url: "http://surrealdb:8000".to_string(),
                ns: "stg_rd_prod".to_string(),
    ...
    fn test_config_environment_methods() {
        env::set_var("RUST_ENV", "development");
        env::set_var("SURREAL_PASSWORD", "dummy");
        let config = Config::load().expect("Failed to load config");
        ...
        env::remove_var("RUST_ENV");
        env::remove_var("SURREAL_PASSWORD");
    }
    ...
    fn test_database_config_structure() {
        ...
        assert_eq!(db_config.url, "http://localhost:50001");

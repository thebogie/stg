...
pub struct ClientAnalyticsController<U> {
    usecase: U,
    db: Db,
}
...
{
    pub fn new(usecase: U, db: Db) -> Self {
        Self { usecase, db }
    }
    ...
    async fn extract_player_id_from_request(
        &self,
        req: &HttpRequest,
        ...
        if let Some(email) = email_opt {
            ...
            if let Some(row) = rows.into_iter().next() {
                if let Some(id) = row.get("id").and_then(|v| v.as_str()) {
                    return Ok(id.replace("player:", "player/"));
                }
            }
    ...
    pub async fn sync_client_data(
        ...
        if player_id != payload.player_id {
            log::warn!(
                "Player ID mismatch: requested {} vs authenticated {}",
                payload.player_id,
                player_id
            );
        ...
        match self.usecase.sync_client_data(&player_id, &payload).await {
            Ok(response) => {
                log::info!(
                    "Client sync successful for player: {}, contests: {}",
                    player_id,
    ...
    pub async fn query_client_analytics(
        ...
        match self
            .usecase
            .query_client_analytics(&player_id, &payload.query)
            .await
        {
            Ok(response) => {
                log::info!(
                    "Client analytics query successful for player: {}",
                    player_id
                );
                Ok(HttpResponse::Ok().json(response))
            }
            Err(e) => {
                log::error!(
                    "Client analytics query failed for player {}: {}",
                    player_id,
                    e
                );
                match e {
    ...
    pub async fn validate_client_data(
        ...
        match self
            .usecase
            .validate_client_data(&player_id, &payload)
            .await
        {
            Ok(response) => {
            ...
            Err(e) => {
                log::error!(
                    "Client data validation failed for player {}: {}",
                    player_id,
                    e
                );
                match e {
...
pub fn configure_routes<U>(
    ...
    _controller: web::Data<ClientAnalyticsController<U>>,
    ...
    cfg.service(
        web::scope(&scope_path)
            ...
            .wrap(crate::auth::AuthMiddleware {
                redis: std::sync::Arc::new((*redis_client).clone()),
            })
            .route(
                "/sync",
                web::post().to(
...
/// Configures enhanced analytics routes. Pass prefix "/api" or "" for Trunk proxy.
pub fn configure_enhanced_routes<U>(
    cfg: &mut web::ServiceConfig,
    controller: web::Data<ClientAnalyticsController<U>>,
    redis_client: std::sync::Arc<redis::Client>,
    prefix: &str,
) where
    U: ClientAnalyticsUseCase + 'static,
{
    let scope_path = if prefix.is_empty() {
        "/analytics-enhanced".to_string()
    } else {
        format!("{}/analytics-enhanced", prefix)
    };
    cfg.service(
        web::scope(&scope_path)
                    ...
                    |_req: HttpRequest,
                     ...
                     controller: web::Data<ClientAnalyticsController<U>>| async move {
                        ...
                        match controller.usecase.get_player_networking(&player_id).await {
                            Ok(networking) => actix_web::HttpResponse::Ok().json(networking),
                            Err(e) => {

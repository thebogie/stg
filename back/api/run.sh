...
validate_environment() {
    ...
    # Check required variables (backend uses SurrealDB)
    local required_vars=(
        "SURREAL_URL"
        "SURREAL_DB"
        "REDIS_URL"
    ...
    case $ENVIRONMENT in
        production)
            ...
            if [ "$SURREAL_PASSWORD" = "test" ] || [ "$SURREAL_PASSWORD" = "REPLACE_WITH_SECURE_PASSWORD" ]; then
                print_error "SURREAL_PASSWORD must be set to a secure value in production!"
                exit 1
            fi
            
            if [[ "$SURREAL_URL" == *"localhost"* ]] || [[ "$SURREAL_URL" == *"127.0.0.1"* ]]; then
                print_error "SURREAL_URL cannot use localhost in production!"
                exit 1
            fi
...
check_database_connectivity() {
    ...
    # Check SurrealDB (HTTP only; if SURREAL_URL is ws://, skip curl check)
    if command -v curl &> /dev/null && [[ "${SURREAL_URL:-}" == http* ]]; then
        if curl -s --connect-timeout 2 -o /dev/null "$SURREAL_URL" 2>/dev/null; then
            print_status "SurrealDB is reachable at $SURREAL_URL"
        else
            print_warning "SurrealDB may not be reachable at $SURREAL_URL (backend uses WebSocket; ensure SurrealDB is running)"
        fi
    else
        print_status "SurrealDB URL: ${SURREAL_URL:-}(not set)"
    fi
...
export SURREAL_URL="${SURREAL_URL:-http://localhost:${SURREALDB_PORT:-50001}}"
...
export SURREAL_NS=${SURREAL_NS:-${SURREAL_DB:-stg_rd}}
...
echo "Database (SurrealDB): $SURREAL_DB (ns: $SURREAL_NS)"
...
echo "SurrealDB User: $SURREAL_USER"

...
use crate::models::{StgVenue, StgGame, StgOutcome, StgContest};
...
async fn main() -> Result<()> {
    ...
    let header_len = reader.read(&mut header)
    ...
    if bytes_read != file_size as usize {
        info!("Warning: Read {} bytes but file size is {} bytes", bytes_read, file_size);
    }
    ...
    info!("Number of opening brackets: {}", content.matches('[').count());
    info!("Number of closing brackets: {}", content.matches(']').count());
    
    // Log the first and last few characters for debugging
    let preview_len = 100;
    info!("First {} characters: {:?}", preview_len, &content[..content.len().min(preview_len)]);
    info!("Last {} characters: {:?}", preview_len, &content[content.len().saturating_sub(preview_len)..]);
        ...
        Err(_e) => {
            ...
            match serde_json::from_str::<serde_json::Value>(&content) {
                            ...
                            if let Some(obj) = value.as_object() {
                                for (key, val) in obj {
                                    ...
                                    match key.as_str() {
                                        "start" | "stop" => {
                                            ...
                                            if let Some(dt_str) = val.as_str() {
                                                match chrono::DateTime::parse_from_rfc3339(dt_str) {
                                                    Ok(_dt) => info!("Successfully parsed {} as DateTime", key),
                                                    Err(e) => info!("Failed to parse {} as DateTime: {}", key, e),
                                                }
                                            }
                                        }
                                        "venue" => {
                                            info!("Venue field: {}", serde_json::to_string_pretty(val).unwrap());
                                        ...
                                        "games" => {
                                            info!("Games field: {}", serde_json::to_string_pretty(val).unwrap());
                                            match serde_json::from_value::<Vec<StgGame>>(val.clone()) {
                                                Ok(_) => info!("Successfully parsed games"),
                                                Err(e) => info!("Failed to parse games: {}", e),
                                            }
                                        }
                                        "outcome" => {
                                            info!("Outcome field: {}", serde_json::to_string_pretty(val).unwrap());
                                            match serde_json::from_value::<Vec<StgOutcome>>(val.clone()) {
                        ...
                        for (i, element) in array.iter().enumerate() {
                            ...
                            match serde_json::from_str::<StgContest>(&element_str) {
                                ...
                                Err(e) => {
                                    ...
                                    return Err(e).context(format!("Failed to convert element {}", i));
                                }
                            }
                        }
                        contests
                    } else {
                        info!("Value is not an array: {}", serde_json::to_string_pretty(&value).unwrap());
                ...
                Err(e) => {
                    ...
                    let line = e.line();
                    let col = e.column();
                    let start = content.lines().take(line.saturating_sub(2)).map(|l| l.len() + 1).sum::<usize>();
                    let end = content.lines().take(line + 2).map(|l| l.len() + 1).sum::<usize>();
                    info!("Error at line {}, column {}", line, col);
                    info!("Error context (lines {} to {}):", line.saturating_sub(2), line + 2);
                    ...
                    if let Some(error_line) = content.lines().nth(line - 1) {
                        ...
                        let line_start = content.lines().take(line - 1).map(|l| l.len() + 1).sum::<usize>();
                        let error_pos = line_start + col - 1;
                        let context_start = error_pos.saturating_sub(10);
                        let context_end = (error_pos + 10).min(content.len());
                        info!("Bytes around error: {:02x?}", content.as_bytes()[context_start..context_end].to_vec());
                    }
